 ...
 * 通用唯一识别码工具类
 * 用于创建32位长唯一数值的字符串工具。
 * 从java1.5库中拷来的代码,为了保证向下兼容才这么作.
 * 增加了一个方法toSimpleString()和toString()方法一样.
 * 只是去掉了4个分割线.
 */
public final class UUID implements java.io.Serializable {
    ...
    private UUID(byte[] data) {
        ...
        long lsb = 0;
        if(data.length != 16){
        	throw new RuntimeException("Make UID ERROR ,");
        }
        for (int i=0; i<8; i++)
    ...
    /**
	 * 获取唯一编码字符串
	 * @return String  唯一的字符编码
	 * */

    public static String getUID() {
		UUID uuid = UUID.randomUUID();
		return uuid.toSimpleString();
	}
}
