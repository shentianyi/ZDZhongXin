...
<script>
	...
	function doClear() {
		...
		$("#three").val(-1);
		$("[name='query.name']").val("");
		$("[name='query.jobnumber']").val("");
		$("[name='query.beginDate']").datebox('clear');
		$("[name='query.endDate']").datebox('clear');
...
<body class="h-100 public" onLoad="doLoad('beginDate,endDate')">
	<div class="public-main abs">
		<div class="ly-contai rel">
			<html:form action="/supervisorOverTime.do" styleId="/supervisorOverTimeForm" method="post" onsubmit="return false">
				...
				<input type="hidden" id="onehiId" value="<c:out value="${supervisorOverTimeForm.query.oneBankId}"/>"/>
				<input type="hidden" id="twohiId" value="<c:out value="${supervisorOverTimeForm.query.twoBankId}"/>"/>
				<input type="hidden" id="threehiId" value="<c:out value="${supervisorOverTimeForm.query.threeBankId}"/>"/>
					...
					<div class="ly-input-w" style="height: 110px;">
						...
						<div class="ly-row clearfix">
							...
							<div class="ly-col fl" style="width:33%;">
								...
								<div class="input block fl hidden">
									<html:text styleClass="ly-bor-none" property="query.jobnumber" styleId="query.jobnumber"/>
								</div>
							</div>
						</div>
	                    <div class="ly-col fl" style="width:66%;">
	                        	<div class="label block fl hidden" style="width:20%;margin-left: -3px;">加班时间：</div>
	                        	<div class="input block fl hidden" style="width:80%;padding-left:28px;">
		                        	<input style="width:24%;" class="ly-bor-none" id="beginDate" name="query.beginDate" type="text" value='<fmt:formatDate value="${msgOutStorageForm.query.beginDate }" pattern="yyyy-MM-dd"/>'/>
		                        	<span>至</span>
		                        	<input style="width:24%;" class="ly-bor-none" id="endDate" name="query.endDate" type="text" value='<fmt:formatDate value="${msgOutStorageForm.query.endDate }" pattern="yyyy-MM-dd"/>'/>
	                        	</div>
	                    	</div>
					</div>
							...
							<tbody class="t-tbody">
								<logic:iterate name="list" id="row" indexId="index">
									<tr class="t-tr" <c:if test="${row.isread==1}">style="color:red;"</c:if>>
										...
										<td class="t-td"><c:out value="${row.jobnumber}" /></td>
										<td class="t-td"><fmt:formatDate value="${row.beginDate}" pattern="yyyy-MM-dd HH:mm:ss" /></td>
										<td class="t-td"><fmt:formatDate value="${row.endDate}" pattern="yyyy-MM-dd HH:mm:ss" /></td>
				...
				<div class="public-main-footer hidden abs">
					...
					<div class="public-main-footer-pagin fr">
						<thumbpage:tools className="<%=ThumbPageConstants.CLASSNAME_DEFAULT.getCode()%>" 
  							tableName="findList" action="/supervisorOverTime.do?method=findList" />
					</div>
				</div>
			</html:form>
		</div>
