...
MzTreeView.prototype.test = function(id){
	alert(this.nodes[id]);
}
...
function MzTreeView(Tname)
{
  ...
  //MzTreeView 初始化入口函数
  this.toString = function(defaultIndex)
  {
    this.browserCheck();
    this.dataFormat();
    this.setStyle();
    ...
    if(rootCN.length>0)
    {
      ...
      for(var i=0; i<rootCN.length; i++)
        ...
        this.name +".focusClientNodeByIndex('"+ defaultIndex +"'); "+ 
		this.name +".atRootIsEmpty()",10);
    }
...
//聚集到客户端生成的某个节点上
//id  客户端树节点的id
MzTreeView.prototype.focusClientNodeByIndex = function(sourceIndex)
{
	this.expandAll();
	for(var i=0; i in this.node;){
		if(this.node[i].sourceIndex==sourceIndex){
			this.focusClientNode(this.node[i].id);
			break;
		}
		i++;
	}
...
//点击展开树节点的对应方法
MzTreeView.prototype.expandEazy   = function(id)
{
  ...
  if (area)
  {
    ...
    var Bool  = area.style.display == "none";
