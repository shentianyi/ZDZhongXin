...
function doSaveno(){
	...
	if(confirm("是否确认审核不通过")){
		...
		if(hasCheckedItem("carIds")){
			...
			ids = ids.substring(0,ids.length - 1);
			document.getElementById("ids").value=ids;
			getElement("method").value="updSuperviseStoragesno";
			document.forms[0].submit();
		} else{
			showMessage("请选择要入库的监管物!");
		}
...
<div class="add_nav">
	<span class="add_nav_acolor">
            ...
            &gt;
            <a class="crumbs-link" style="color:#929291;" href="#">监管物入库审批</a>
         </span>
</div>
<div class="public-main abs">
	<div class="ly-contai rel">
		<html:form action="/carOperation" styleId="carOperationForm" method="post" onsubmit="return false">
			<input name="method" id="method" type="hidden" value="storageList"/>
			...
			<div class="public-main-input ly-col-2  abs"style="margin-top:-10px;">
					...
					<div class="ly-row clearfix">
	                    ...
	                    <div class="ly-col fl">
	                        ...
	                        <div class="input block fl hidden  add_easyui">
								<select id="superviseImportquery.brandid" style="width:100px;" name="superviseImportquery.brandid" >
										<option value="-1">请选择</option>
										<c:forEach items="${brandOptions}" var="row">
											<option <c:if test="${carOperationForm.superviseImportquery.brandid==row.value}">selected='selected'</c:if>  value="<c:out value='${row.value }'/>"><c:out value="${row.label }"/></option> 
										</c:forEach>
								</select>
	                        </div>
	                    </div>
				...
				<div class="ly-button-w">
	                ...
	                <c:if test="${list!=null&&summary!=null&&summary.count!=0 }">
						<span style="position: absolute;top:75%;left:60%;">
						 总台数:<c:out value="${summary.count }"/>
						...
						<tbody class="t-tbody hidden">
							<logic:iterate name="list" id="row" indexId="index">
								<tr class="t-tr">
									...
									<td class="t-td"><select:bankaprove sid="${row.id}" idtype="storagetime"/></td>
								</tr>
							</logic:iterate>
						</tbody>
					</table>
			...
			<div class="public-main-footer hidden abs">
				...
				<div class="public-main-footer-pagin fr">
					<thumbpage:tools className="<%=ThumbPageConstants.CLASSNAME_DEFAULT.getCode()%>" tableName="storageList" action="/carOperation.do?method=storageList"/>
				</div>
			</div>
		</html:form>
	</div>
