		...
		<html:form action="/message/supMaMsg" styleId="messageForm" method="post" onsubmit="return false">
			...
			<div class="ly-input-w">
				<div class="ly-row clearfix">
                    ...
                    <div class="ly-col fl">
                        <div class="label block fl hidden">离职时间：</div>
                        <div class="input block fl hidden">
                        	<form:calendar property="supMaQuery.leaveTime" styleId="leaveTime" pattern="<%=PatternConstants.TIMESTAMP.getCode()%>" readonly="true" />
                        </div>
                    </div>
                    <div class="ly-col fl">
                        <div class="label block fl hidden">离岗时间：</div>
                        <div class="input block fl hidden">
                        	<form:calendar property="supMaQuery.dimissionDate" styleId="dimissionDate" pattern="<%=PatternConstants.TIMESTAMP.getCode()%>" readonly="true" />
                        </div>
                    </div>
				</div>
			</div>
					...
					<table class="t-table" border="0" cellspacing="0" cellpadding="0">
						<thead class="t-thead">
							<tr class="t-tr">
								...
								<th class="t-th">离职时间</th>
								<th class="t-th">离岗时间</th>
								<th class="t-th">
						...
						<tbody class="t-tbody hidden">
							<logic:iterate name="list" id="row" indexId="index">
								<tr class="t-tr">
									...
									<td class="t-td">
										<font <c:if test="${row.isread == 1}">color="red"</c:if>>
											<select:timestamp timestamp="${row.entryTime}" idtype="date"/>
										</font>
									</td>
									<td class="t-td">
										<font <c:if test="${row.isread == 1}">color="red"</c:if>>
											<select:timestamp timestamp="${row.leaveTime}" idtype="date"/>
										</font>
									</td>
									<td class="t-td">
										<font <c:if test="${row.isread == 1}">color="red"</c:if>>
											<select:timestamp timestamp="${row.dimissionDate}" idtype="date"/>
										</font>
									</td>
									<td class="t-td">
