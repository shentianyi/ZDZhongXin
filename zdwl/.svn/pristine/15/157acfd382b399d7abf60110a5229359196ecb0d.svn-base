...
function doSave(){
	...
	if(hasCheckedItem("resourceIds")){
		...
		getElement("method").value="addResourceRole";
		document.forms[0].submit();
	} else{
		showMessage("请选择要添加的资源!");
	}
}

//进入角色下已分配资源页面
function goResourceWithRole(){
	location = "<url:context/>/rbac/resource.do?method=resourceListWithRole&roleId=<c:out value="${roleId}"/>&<thumbpage:setQueryFlag tableName="resourceListWithRole" out="true"/>";
}
	...
	<div class="ly-contai clearfix">
        <div class="public-bar-tab fr hidden clearfix">
            <div class="ly-button-w fr">
                <a class="button fl" href="javascript:goResourceWithRole();">已分配资源</a>
                <a class="button btn-sel fl" href="javascript:void(0);">分配新资源</a>
            </div>
        </div>
	</div>
</div>
<div class="public-main abs">
	<div class="ly-contai rel">
		<html:form action="/rbac/resource" styleId="resourceForm" method="post" onsubmit="return false">
			<input type="hidden" name="method" id="method" value="searchResourceWithoutRole" />
			...
			<div class="public-main-footer hidden abs">
				...
				<div class="public-main-footer-pagin fr">
					<thumbpage:tools className="<%=ThumbPageConstants.CLASSNAME_TAGBAR.getCode()%>" tableName="searchResourceWithoutRole" action="/rbac/resource.do?method=searchResourceWithoutRole"/>
				</div>
			</div>
		</html:form>
	</div>
