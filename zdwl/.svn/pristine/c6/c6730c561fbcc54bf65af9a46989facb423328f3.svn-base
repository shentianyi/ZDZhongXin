...
function doSave(){
	...
	var ismaskMessageIds=$("[name='ismaskMessageIds']:checked");
	var ismaskIds=[];
	ismaskMessageIds.each(function(){
		ismaskIds.push(this.value);
	});
	ismaskIds = ismaskIds.join();
	document.getElementById("ismaskIds").value = ismaskIds;
	
	if(messageIds.length<=0 && ismaskMessageIds.length<=0){
		alert("请选择要标记为已读或屏蔽的消息!");
		return false;
	}
...
<div class="public-main abs">
	<div class="ly-contai rel">
		<html:form action="/message/supMaMsg" styleId="messageForm" method="post" onsubmit="return false">
			...
			<input type="hidden" id="ismaskIds" name="ismaskIds" />
			...
			<div class="ly-input-w">
				<div class="ly-row clearfix">
                    ...
                    <div class="ly-col fl">
                        <div class="label block fl hidden">地址：</div>
                        <div class="input block fl hidden">
                        	<html:text styleClass="ly-bor-none" property="supMaQuery.address" styleId="address"/>
                        </div>
                    </div>
                    <div class="ly-col fl">
                        <div class="label block fl hidden">辞职发起时间：</div>
                        <div class="input block fl hidden">
                        	<form:calendar property="supMaQuery.resignApplyDate" styleId="resignApplyDate" pattern="<%=PatternConstants.TIMESTAMP.getCode()%>" readonly="true" />
                        </div>
                    </div>
                    <div class="ly-col fl">
                        <div class="label block fl hidden">预计离职时间：</div>
                        <div class="input block fl hidden">
                        	<form:calendar property="supMaQuery.expectResignTime" styleId="expectResignTime" pattern="<%=PatternConstants.TIMESTAMP.getCode()%>" readonly="true" />
                        </div>
                    </div>
				</div>
			</div>
					...
					<table class="t-table" border="0" cellspacing="0" cellpadding="0">
						<thead class="t-thead">
							<tr class="t-tr">
								...
								<th class="t-th">地址</th>
								<th class="t-th">辞职发起时间</th>
								<th class="t-th">预计离职时间</th>
								<th class="t-th">
								...
								<th class="t-th">
									<input name="test" value="" onclick="if(this.checked==true) { checkAll('ismaskMessageIds'); } else { clearAll('ismaskMessageIds'); }" type="checkbox">
									<label for="checkAllValue">屏蔽</label>
								</th>
							</tr>
						</thead>
						<tbody class="t-tbody hidden">
							<logic:iterate name="list" id="row" indexId="index">
								<tr class="t-tr">
									...
									<td class="t-td">
										<font <c:if test="${row.isread == 1}">color="red"</c:if>>
											<select:timestamp timestamp="${row.resignApplyDate}" idtype="date"/>
										</font>
									</td>
									<td class="t-td">
										<font <c:if test="${row.isread == 1}">color="red"</c:if>>
											<select:timestamp timestamp="${row.expectResignTime}" idtype="date"/>
										</font>
									</td>
									<td class="t-td">
									...
									<td class="t-td">
										<c:if test="${row.ismask == 1}">
											<input type="checkbox" id="<c:out value='${row.id }'/>ismaskcheckbox" name="ismaskMessageIds" value="<c:out value='${row.id}'/>"/>
										</c:if>
										<label>屏蔽</label>
									</td>
								</tr>
