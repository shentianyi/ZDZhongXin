...
function init(){
	...
	//进入新增页面
	function goAdd() {
		location = "<url:context/>/supervisor/checkStockManage.do?method=preAdd";
	}

	//进入修改页面
	function goUpd(id) {
		location = "<url:context/>/supervisor/checkStockManage.do?method=preUpdate&checkStock.id=" + id;
	}

	//执行删除操作
	function doDel(id) {
		if (confirm("删除后数据不可恢复\n是否继续？")) {
			location = "<url:context/>/supervisor/checkStockManage.do?method=delete&checkStock.id=" + id;
		}
	}
	function doSubmit(id){
		if(confirm("确定要提交吗？")){
			location.href="<url:context/>/supervisor/checkStockManage.do?method=submit&checkStock.id="+id;
		}
	}
	
	function detail(id){
		location.href="<url:context/>/supervisor/checkStockManage.do?method=detailPage&checkStock.id="+id;
	}
...
<div class="add_nav">
	<span class="add_nav_acolor">
            ...
            &gt;
            <a class="crumbs-link" style="color:#929291;" href="#">日查库管理新</a>
         </span>
</div>
<div class="public-main abs">
	<div class="ly-contai rel">
		<c:set var="supervisory"><%=RoleConstants.SUPERVISORY.getCode() %></c:set>
		<html:form action="/supervisor/checkStockManage.do" styleId="checkStockManageForm" method="post" onsubmit="return false">
					...
					<div class="ly-row clearfix">
	                    ...
	                    <div class="ly-col fl">
	                       <div class="label block fl hidden">品牌：</div>
	                        <div class="input block fl hidden">
	                        	<html:text styleClass="ly-bor-none" property="query.brandName" styleId="brandName"/>
	                        </div>
	                    </div>
	                    <div class="ly-col fl">
	                        <div class="label block fl hidden">经销商属性：</div>
	                        <div class="input block fl hidden">
	                        	<html:text styleClass="ly-bor-none" property="query.dealerAttr" styleId="dealerAttr"/>
	                        </div>
	                    </div>
					</div>
					<div class="ly-row clearfix">
						<div class="ly-col fl" style="width: 50%">
	                        <div class="label block fl hidden"style="width: 15%">时间：</div>
	                        <div class="input block fl hidden" style="width: 85%">
	                        	<input style="width: 35%" id="startTime" name="query.beginTime" type="text"/>
							 ——<input style="width: 35%" id="endTime" name="query.endTime" type="text"/> 
	                        </div>
	                    </div>
	                    <div class="ly-col fl">
	                       <div class="label block fl hidden">查库方式：</div>
	                        <div class="input block fl hidden">
	                        	<select style="width:80%;margin-top:4px;margin-left:18px;"Class="ly-bor-none" name="query.checkStockType" >
									<option value="-1">请选择</option>
									<option value="1">手工查库</option>
									<option value="2">设备查库</option>
								</select>
	                        </div>
	                    </div>
					</div>
					...
					<table class="t-table" border="0" cellspacing="0" cellpadding="0">
						<thead class="t-thead">
							<tr class="t-tr">
								...
								<th class="t-th">品牌</th>
								<th class="t-th">经销商属性</th>
								<th class="t-th">创建人</th>
								<th class="t-th">创建时间</th>
								<th class="t-th">创建查库方式</th>
						      	<th class="t-th">操作</th>
							</tr>
						</thead>
			...
			<div class="public-main-footer hidden abs">
				...
				<div class="public-main-footer-pagin fr">
					<thumbpage:tools className="<%=ThumbPageConstants.CLASSNAME_DEFAULT.getCode()%>" tableName="list" action="/supervisor/checkStockManage.do?method=findList" />
				</div>
			</div>
		</html:form>
	</div>
