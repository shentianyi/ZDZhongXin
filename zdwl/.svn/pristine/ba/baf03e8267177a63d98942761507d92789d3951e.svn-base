...
<script>
	...
	//执行返回列表操作
	function doReturn() {
		location = "<url:context/>/inspectionPlan.do?method=findListAlreadyCheck";
	}
...
<%-- 										<td class="t-td"><c:out value="${row.mkTableUserId}" /></td> --%>
										...
										<td class="t-td"><input style="width: 60% !important; height: 78%;" 
				...
				<div style="margin-top:20px; overflow: hidden;">
					<div class="public-main-footer-pagin fr">
						<thumbpage:tools className="<%=ThumbPageConstants.CLASSNAME_DEFAULT.getCode()%>" 
  							tableName="detail" action="/inspectionPlan.do?method=detailAlreadyCheck" />
					</div>
				</div>
