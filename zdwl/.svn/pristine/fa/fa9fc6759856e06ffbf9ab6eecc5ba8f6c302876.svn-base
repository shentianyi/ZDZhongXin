...
<script>
	...
	//已审核列表
	function goApprove() {
		location.href = "<url:context/>/videoPlan.do?method=findListApprove";
	}
	
	//进入详情页面
	function detail(id,planCode,status) {
		location = "<url:context/>/videoPlan.do?method=detailApproveno&videoPlan.id=" + id + 
		...
		<div class="ly-contai clearfix">
			<div class="public-bar-tab fr hidden clearfix">
				<div class="ly-button-w fr">
					<a class="button fl" href="javascript:goApprove();">未审核</a> 
						...
						<div class="ly-row clearfix">
							...
							<div class="ly-col fl">
								<div class="label block fl hidden">支行：</div>
									<div class="input block fl hidden">
									<div class="ly-sel-w">
										<select id="three" name="query.threeBankId" style="height:24px;padding: 0 auto;margin-top:-1px;" class="ly-bor-none threeBankId">
											<option value="-1">请选择...</option>
										</select>
									</div>
								</div>
							...
							<div class="ly-col fl">
								...
								<div class="input block fl hidden">
									<select id="provinceId" name="query.provinceId" style="height:24px;padding: 0 auto;margin-top:5px;" class="ly-bor-none provinceId" onchange="changeProvince(this.value,$('#cityId'))">
										<option value="-1">请选择...</option>
									</select>
								</div>
							</div>
...
<%-- 										<td class="t-td"><c:out value="${row.unCheckPassCause}" /></td> --%>
										<td class="t-td">
											...
											,'<c:out value="${row.planCode }"/>','<c:out value="${row.status }"/>')">查看</a>
										</td>
									</tr>
								</logic:iterate>
							</tbody>
...
<!-- 					<a href="javascript:goAdd();" class="button btn-add fl">新增</a> -->
					<div class="public-main-footer-pagin fr">
						<thumbpage:tools className="<%=ThumbPageConstants.CLASSNAME_DEFAULT.getCode()%>" 
 							tableName="list" action="/videoPlan.do?method=findListApproveno" />
					</div>
				</div>
			</html:form>
		</div>
