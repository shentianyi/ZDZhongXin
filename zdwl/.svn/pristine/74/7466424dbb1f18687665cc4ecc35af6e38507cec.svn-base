...
<script>
	...
	//执行返回列表操作
	function doReturn() {
		location = "<url:context/>/videoPlan.do?method=findListApprove";
	}
	...
	//审批通过
	function approve(){
		var unCheckPassCause = $("[name='videoPlanInfo.unCheckPassCause']").val();
		if(confirm("确认审核通过？")){
...
<body class="h-100 public" onLoad="doLoad()">
	...
	<div class="public-main abs">
		<div class="ly-contai rel" style="margin-top: -30px;margin-left:0%;height: 610px;width:99%;">
<!-- 		<div class="ly-contai rel" style="margin: -60px 30px; height: 610px;"> -->
...
<%-- 										<td class="t-td"><c:out value="${row.practicalCheckTime}" /></td> --%>
				...
				<div style="margin-top:20px;">
					<div class="public-main-footer-pagin fr">
						<thumbpage:tools className="<%=ThumbPageConstants.CLASSNAME_DEFAULT.getCode()%>" 
  							tableName="detail" action="/videoPlan.do?method=detailApprove" />
					</div>
				</div>
				<br>
				<br />
				...
				<div>
					<html:textarea property="videoPlanInfo.unCheckPassCause" styleId="unCheckPassCause"
				...
				<br/>
				<!-- 新增 -->
				<c:if test="${status==3 || status==4 || status==5 }">
                    <jsp:include page="/WEB-INF/jsp/approvalRecord/approvalRecord.jsp"/>
                </c:if>
			</html:form>
		</div>
