...
function goAdd(){
	...
	window.location="<url:context/>/supervisory.do?method=addSupervisoryFamilyEntity&baseInfo.id="+id;
}

//进入修改页面
function goUpd(id){
	window.location="<url:context/>/supervisory.do?method=updSupervisoryFamilyEntity&family.id="+id;
}
...
function doDel(id){
	if(confirm("确认删除？")){
		...
		window.location="<url:context/>/supervisory.do?method=delSupervisoryFamily&family.id="+id+"&baseInfo.id="+baseInfoId;
	}
}
...
<div class="title">监管员家庭状况管理</div>
<div class="pagebodyOuter">
	<div class="pagebodyInner">
	
<html:form action="/supervisory" styleId="supervisoryForm" method="post" onsubmit="return false">
<input name="method" id="method" type="hidden" value="getFamilyListBySupervisoryId"/>
