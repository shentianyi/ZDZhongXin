...
function doSaveno(){
	...
	if(confirm("是否确认审核不通过")){
		if(hasCheckedItem("carIds")){
			...
			document.getElementById("ids").value=ids;
			getElement("method").value="updSuperviseMovesno";
			document.forms[0].submit();
		} else{
			showMessage("请选择要移动的监管物!");
		}
...
<div class="add_nav">
	<span class="add_nav_acolor">
            ...
            &gt;
            <a class="crumbs-link" style="color:#929291;" href="#">监管物移动审批</a>
         </span>
</div>
<div class="public-main abs">
	<div class="ly-contai rel">
		<html:form action="/carOperation" styleId="carOperationForm" method="post" onsubmit="return false">
			<input name="method" id="method" type="hidden" value="moveList"/>
			...
			<div class="public-main-input ly-col-2   abs"style="margin-top:-10px;">
				...
				<div class="ly-button-w">
	                ...
	                <c:if test="${list!=null&&summary!=null&&summary.count!=0 }">
						...
						&nbsp;&nbsp;&nbsp;总保证金:<fmt:formatNumber value="${summary.carBond }" pattern="#.##" minFractionDigits="2" />
						</span>
					</c:if>
	            </div>
			</div>
						...
						<tbody class="t-tbody hidden">
							<logic:iterate name="list" id="row" indexId="index">
								<tr class="t-tr">
									...
									<td align="center"><select:timestamp timestamp="${row.storagetime}" idtype="ss"/></td>
									<td class="t-td"><select:bankaprove sid="${row.id}" idtype="movetime"/></td>
			...
			<div class="public-main-footer hidden abs">
				...
				<div class="public-main-footer-pagin fr">
					<thumbpage:tools className="<%=ThumbPageConstants.CLASSNAME_DEFAULT.getCode()%>" tableName="moveList" action="/carOperation.do?method=moveList"/>
				</div>
			</div>
		</html:form>
	</div>
