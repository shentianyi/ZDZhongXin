...
<script src="/js/inspect/list.js"></script>
<style type="text/css">
.title td {
	padding: 0 15px 0 15px;
}
.oneId, .twoId, .twoId, .prodirectorceId, .cityId {
	margin: 3% 10% ;
	width: 50% ;
    height: 64%;
}
...
<script>
	...
	function doClear() {
		...
		$("#cityId").val(-1);
		$("#areaId").val(-1);
		$("[name='query.director']").val("");
		$("#lastModified").datebox('clear');
		$("#endLastModified").datebox('clear');
...
<body class="h-100 public" onLoad="doLoad('lastModified,endLastModified');doLoadArea()">
	<div class="public-main abs">
		<div class="ly-contai rel">
			<html:form action="/unInspect.do" styleId="/unInspectFrom" method="post" onsubmit="return false">
				...
				<input type="hidden" id="areaiId"
					value="<c:out value="${inspectionForm.query.areaId}"/>" />
					...
					<div class="ly-input-w" style="height: 110px;">
						...
						<div class="ly-col fl">
							...
							<div class="ly-col fl">
								...
								<div class="input block fl hidden">
									<select id="cityId" name="query.cityId"
											class="cityId" style="margin: 0;"
											onchange="changeProvince(this.value,$('#areaId'))">
											<option value="-1">请选择...</option>
									</select>
								</div>
							</div>
							<div class="ly-col fl">
								<div class="label block fl hidden">区：</div>
								<div class="input block fl hidden">
									<select id="areaId" name="query.areaId" class="areaId">
										<option value="-1">请选择...</option>
									</select>
								</div>
							</div>
	                    ...
	                    <div class="ly-col fl" style="width:66%;" >
	                        	<div class="label block fl hidden" style="width:20%;margin-left: -3px;">上次巡店报告上传时间：</div>
	                        	<div class="input block fl hidden" style="width:80%;padding-left:28px;">
		                        	<input style="width:24%;" class="ly-bor-none" id="lastModified" name="query.lastModified" type="text" value='<fmt:formatDate value="${unInspectFrom.query.lastModified }" pattern="yyyy-MM-dd"/>'/>
	                        		<span>至</span>
		                        	<input style="width:24%;" class="ly-bor-none" id="endLastModified" name="query.endLastModified" type="text" value='<fmt:formatDate value="${unInspectFrom.query.endLastModified }" pattern="yyyy-MM-dd"/>'/>
	                        	</div>
	                    </div>
	                    </div>
					</div>
							...
							<tbody class="t-tbody">
								<logic:iterate name="list" id="row" indexId="index">
									<tr class="t-tr" <c:if test="${row.isread==1}">style="color:red;"</c:if>>
										...
										<td class="t-td"><c:out value="${row.content}" /></td>
