        ...
        function ajaxLoopRequest(formData, ii) {
            ...
            var ajaxOptions = $.extend({}, {//store the request to the uploadList variable
                ...
                error: function (request, error) {
                	thisS.language.serverError = request.responseText;
                    if (error !== 'abort') {
...
//                        setElementMessage(thisS, ii, 'error', msg, request.responseText);
            ...
            $.ajax(ajaxOptions).done(function (responseData, textStatus, jqXHR) {
            	...
            	/*if($("#dataType").val()=="text"&&data.indexOf("上传失败")==0){
    ...
    var locale = {
        en: {
            ...
            clear: '清空',
            drag: '将图片拖拽至此并释放',
            sizeError: '$1 超出了 $2 的大小限制',// $1=file name ,$2=max ie( example.jpg has has exceed the size limit of 2mb)
            extError: '$1 类型不被支持',//$1=file extension ie(exe files are not supported)
