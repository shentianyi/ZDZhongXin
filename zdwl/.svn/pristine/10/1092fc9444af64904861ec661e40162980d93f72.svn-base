		...
		<span class="add_nav_acolor">
            <a class="crumbs-link" style="color:#929291;" href="#">系统管理与配置</a>
             ...
             <a class="crumbs-link" style="color:#929291;" href="#">角色管理</a>
              &gt;
              <a class="crumbs-link" style="color:#929291;" href="#">修改角色</a>
         </span>
	</div>
...
<html:hidden property="role.id" styleId="role.id"/>
<html:hidden property="role.state" styleId="role.state"/>
<input type="hidden" name="method" id="method" value="updRole">
...
<table class="formTable">
	<tr>
		...
		<select id="role.client_type" name="role.client_type" >
										...
										<c:forEach items="${clientTypeOptions }" var="row">
											<option <c:if test="${roleForm.role.client_type==row.value}">selected='selected'</c:if>  value="<c:out value='${row.value }'/>"><c:out value="${row.label }"/></option> 
										</c:forEach>
			</select>
		</td>
	</tr>
	<tr>
		...
		<td class="codeCol" colspan="3">
			<html:textarea property="role.des" styleId="role.des" rows="4" cols="70"></html:textarea>
		</td>
	</tr>
