...
//进入修改页面
function goUpd(id){
	location = "<url:context/>/superviseMove.do?method=updSuperviseMoveEntry&superviseImport.id="+id;
}

//执行删除操作
function doDel(id){
	if(confirm("删除后数据不可恢复\n是否继续？")){
		location="<url:context/>/superviseMove.do?method=delSuperviseMove&superviseImport.id="+id;
	}
}
...
function doClear(){
	...
	getElement("superviseImportquery.two_name").value="";
...
function doSave(){
	...
	var two_name = $(carIds[0]).attr('ewName');
	var flag = true;
	carIds.each(function(){
		...
		var bond = $(this).attr('bond');
		var vin = $(this).attr('vin');
		if (!bond) {
			alert("车架号为"+vin+"的车没有保证金，不能移动！");
			flag=false;
			return flag;
		}
		ids.push(this.value);
	...
	$.ajax({
	   type: "get",
	   url: "/json/checkSupervise.do?callback=?&dealerId="+dealerId+"&ids="+ids,
	   dataType:"jsonp",
	   success: function(result){
		   if(result.success){
			   document.getElementById("ids").value = ids;
				$("#chooseTwoAddress iframe").attr("src","/superviseMove.do?method=chooseTwoAddress&dealerId="+dealerId+"&ewName="+two_name);
				$('#chooseTwoAddress').dialog({
				    title: '选择二网名称',
				    width: 800,
				    height: 400,
				    closed: false,
...
function closeDialog(ta){
	document.getElementById("ta").value=ta;
	getElement("method").value="updSuperviseMoves";
	document.forms[0].submit();
}
function goNow(){
	location.href="<url:context/>/superviseMove.do?method=superviseMoveNowList";
}

var flag=false;
$(function(){
	$(".bondNumberBox").each(function(){
		$(this).numberbox({    
		    min:0,    
		    precision:2
		});
		$(this).next('span').find('input:first').blur(function(){//失去焦点
			...
			$.ajax({
				url:"/superviseMove.do?method=updatebond&id="+td.attr("carid")+"&bond="+this.value,
				type:"get",
				dataType:"json",
				success:function(result){
					if(result.success){
						...
						$("#"+td.attr("carid")+"checkbox").attr('bond',thisValue);
			    		$(".numberbox").hide();
			    		flag=false;
			    		document.forms[0].submit();
					}else{
	...
	$(".updateBond").click(function(){
		...
		var bond = $(this).find('span:first').html();
		$(this).find('span:first').html("");
		$('#'+id+'updatebond').numberbox('setValue',bond);
		$('#'+id+'updatebond').next('span').show().find('input').focus();
	});
});
$(function(){
//限制超级角色操作
...
<div class="add_nav">
	<span class="add_nav_acolor">
            ...
            &gt;
            <a class="crumbs-link" style="color:#929291;" href="#">监管物移动</a>
         </span>
</div>
	...
	<div class="ly-contai clearfix">
        <div class="public-bar-tab fr hidden clearfix">
            <div class="ly-button-w fr">
                <a class="button btn-sel fl" href="javascript:void(0);">监管物移动</a>
                <a class="button fl" href="javascript:goNow();">监管物移动申请中</a>
            </div>
        </div>
	</div>
</div>
<div class="public-main abs">
	<div class="ly-contai rel">
		<html:form action="/superviseMove" styleId="superviseImportForm" method="post" onsubmit="return false">
			<input name="method" id="method" type="hidden" value="superviseMoveList"/>
			<input type="hidden" id="ids" name="ids" />
			<input type="hidden" id="ta" name="ta" />
			<input type="hidden" id="bigger" name="bigger" />
			<div class="public-main-input ly-col-2 hidden abs">
				<div class="ly-input-w" style="margin-top:30px;margin-bottom:-10px">
					...
					<div class="ly-row clearfix">
	                    ...
	                    <div class="ly-col fl">
	                        <div class="label block fl hidden">二网名称：</div>
	                        <div class="input block fl hidden">
	                            <html:text styleClass="ly-bor-none" property="superviseImportquery.two_name" styleId="superviseImportquery.two_name"/>
	                        </div>
	                    </div>
	                    <div class="ly-col fl">
							...
							<logic:iterate name="list" id="row" indexId="index">
								<tr class="t-tr">
									<td class="t-td">
										...
										bond="<c:out value="${row.bond}"/>"
										vin="<c:out value="${row.vin}"/>"
										ewName="<c:out value="${row.two_name }"/>"
										/>
									</td>
									...
									<td class="t-td updateBond" carid="<c:out value='${row.id }'/>">
										<span><c:out value="${row.bond}" /></span>
										<input style="width:100px;" class="bondNumberBox" type="text" id="<c:out value='${row.id }'/>updatebond">
									</td>
			...
			<div class="public-main-footer hidden abs">
				<a href="javascript:doSave();" class="button btn-add fl yc">移动</a>
				<div class="public-main-footer-pagin fr">
					<thumbpage:tools className="<%=ThumbPageConstants.CLASSNAME_DEFAULT.getCode()%>" tableName="superviseMoveList" action="/superviseMove.do?method=superviseMoveList"/>
				</div>
			</div>
		</html:form>
	</div>
