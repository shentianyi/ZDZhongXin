 ...
 *
 */
public class Billing7nocar {

	public static void billing7(){
			...
			if(dList != null && dList.size() > 0){
				for(DraftVO dvo : dList){
					...
					if(sList == null || sList.size() == 0){
						...
						if(day > 7){
							MessageUtil.sendMsg(new String[]{RoleConstants.BUSINESS_COMMISSIONER.getCode()+""}, "票号为"+dvo.getDraft_num()+"的汇票开票7个工作日未到车",
									"/message/BillingNoCarMessage.do?method=billno7&draftnum="+dvo.getDraft_num(), 1, MessageTypeContant.BILLING7NOCAR.getCode(), dvo.getCreateuserid());
						}
					}
				}
			}
