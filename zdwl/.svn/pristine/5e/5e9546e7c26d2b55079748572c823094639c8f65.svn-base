...
//执行返回列表操作
function doReturn(){
	location = "<url:context/>/ledger/leaveApply.do?method=findPageList";
}
...
<html:form action="/ledger/leaveApply" styleId="leaveApplyForm" method="post" onsubmit="return false" enctype="multipart/form-data">
						...
						<tbody onMouseOver="mouseOver()" onMouseOut="mouseOut()" onClick="clickRow()">
							<logic:iterate name="leaveReplaceDynamicList" id="row" indexId="index" >
								<tr class="listTr_a">
									...
									<td >
										<select:timestamp timestamp="${row.replaceStartTime}" idtype="date" />
									</td>
									<td >
										<select:timestamp timestamp="${row.replaceEndTime}" idtype="date" />
									</td>
									<td >
