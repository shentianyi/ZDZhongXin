		...
		<div class="ly-contai clearfix">
			<div class="public-bar-tab fr hidden clearfix">
				<div class="ly-button-w fr">
					<a class="button btn-sel fl" href="javascript:void(0);">未完成</a> 
					<a class="button fl" href="javascript:completed();;">已完成</a>
				</div>
			</div>
		</div>
	</div>
	<div class="public-main abs">
		<div class="ly-contai rel">
			<html:form action="/windcontrol/inspection.do" styleId="inspectionForm"
				...
				<input name="method" id="method" type="hidden" value="findProgressList" />
								...
								<c:if test="${!empty list}">
									<logic:iterate name="list" id="row" indexId="index">
										<tr class="t-tr">
											...
											<td class="t-td">
											 <c:if test="${row.completeProcess==0}">未编辑</c:if>
											 <c:if test="${row.completeProcess==1}">已编辑</c:if> 
											</td>
											<%-- <td class="t-td yc"><a
												href="javascript:goUpd('<c:out value="${row.id }"/>')">编辑</a>
											</td> --%>
											<td class="t-td yc">
											<a href="javascript:goUpd('<c:out value="${row.id }"/>','<c:out value="${row.dealerName}" />')">编辑</a>
                                            </td>
				...
				<div class="public-main-footer hidden abs">
					...
					<div class="public-main-footer-pagin fr">
						<thumbpage:tools
							...
							tableName="pagelist" action="/windcontrol/inspection.do?method=findProgressList" />
					</div>
				</div>
			</html:form>
		</div>
