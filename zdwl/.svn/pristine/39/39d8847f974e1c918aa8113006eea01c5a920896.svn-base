...
//进入新增页面
function goAdd(){
	window.location="<url:context/>/repository.do?method=addRepositoryEntity";
}

//进入修改页面
function goUpd(id){
	window.location="<url:context/>/repository.do?method=updRepositoryEntity&repository.id="+id;
}

//进入详情页面
function goDetail(id){
	window.location="<url:context/>/repository.do?method=repositoryDetailEntity&repository.id="+id;

}
//进入可驻派城市列表页
function goDispatchCityList(id){
	window.location="<url:context/>/repository.do?method=getRepositoryDispatchCityByRepositoryId&repository.id="+id;
}
//进入修改培训信息页面
function goUpdRepositoryTrain(id){
	window.location="<url:context/>/repository.do?method=updRepositoryTrainEntity&repository.id="+id;
}

//执行删除操作
function doDel(id){
	if(confirm("确认删除？")){
		window.location="<url:context/>/repository.do?method=delRepository&repository.id="+id;
	}
}
//导出
function goExport(){
	...
	//request.setAttibute("status",status);
}
//执行查询操作
function doQuery(){
	$("#method").val("repositoryPageList");
	document.forms[0].submit();
}
...
<div class="public-main abs">
	<div class="ly-contai rel">
		...
		<input name="method" id="method" type="hidden" value="repositoryPageList"/>
						...
						<logic:iterate name="list" id="row" indexId="index">
							<tr class="t-tr">
								...
								<td class="t-td">
									...
									<c:choose>
										<c:when test="${role == '4'}">
											<a href="javascript:goDispatchCityList('<c:out value='${row.repository.id}'/>');">可驻派城市</a>&nbsp;
											<a href="javascript:goUpd('<c:out value='${row.repository.id}'/>');">修改</a>&nbsp;
											<a href="javascript:doDel('<c:out value='${row.repository.id}'/>');">删除</a>&nbsp;
										</c:when>
									</c:choose>
									<c:choose>
										<c:when test="${role == '7'}">
											<a href="javascript:goUpdRepositoryTrain('<c:out value='${row.repository.id}'/>');">培训信息</a>&nbsp;
										</c:when>
									</c:choose>
								</td>
							</tr>
		...
		<div class="public-main-footer hidden abs">
			...
			<div class="public-main-footer-pagin fr">
				<thumbpage:tools className="<%=ThumbPageConstants.CLASSNAME_DEFAULT.getCode()%>" tableName="repositoryList" action="/repository.do?method=repositoryPageList"/>
			</div>
		</div>
		</html:form>
	</div>
