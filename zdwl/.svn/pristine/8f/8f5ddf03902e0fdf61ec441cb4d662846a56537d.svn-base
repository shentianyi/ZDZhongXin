...
//进入新增页面
function goAdd(){
	location="<url:context/>/addresslist.do?method=addAddresslistEntry";
}

//进入修改页面
function goUpd(id){
	location = "<url:context/>/addresslist.do?method=updAddresslistEntry&addresslist.id="+id;
}

//执行删除操作
function doDel(id){
	if(confirm("删除后数据不可恢复\n是否继续？")){
		location="<url:context/>/addresslist.do?method=delAddresslist&addresslist.id="+id;
	}
}
...
function doQuery(){
	...
	if(validateMain("addresslistForm")){
		//为时间类型输入项补齐时间戳
		setTimeSuffix();
		//提交表单
...
function doClear(){
	...
	getElement("addresslistquery.phone").value="";
	getElement("addresslistquery.landline").value="";
	getElement("addresslistquery.email").value="";
	getElement("addresslistquery.fax").value="";
	getElement("addresslistquery.qq").value="";
	getElement("addresslistquery.duty").value="";
}
</script>
</head>
<body onLoad="doLoad()">
...
<input name="method" id="method" type="hidden" value="addressList"/>
<input type="hidden" id="addresslistquery.genre" name="addresslistquery.genre" value="1" />
<!-- 查询条件 -->
<table class="formTable" cellpadding="0" cellspacing="0">
	...
	<tr>
		<td class="nameCol">手机号码：</td>
		<td class="codeCol"><html:text property="addresslistquery.phone" styleId="addresslistquery.phone"/></td>
		...
		<td class="nameCol">邮箱：</td>
	  	<td class="codeCol"><html:text property="addresslistquery.email" styleId="addresslistquery.email"/></td>
	</tr>
	<tr>
		...
		<td class="nameCol">职责：</td>
	  	<td class="codeCol">
	  		<html:text property="addresslistquery.duty" styleId="addresslistquery.duty"/>
	  	</td>
	</tr>
	<!-- 查询按钮 -->
...
<table  class="listTalbe" cellpadding="0" cellspacing="0">
	<thead>
		<tr class="title">
	      ...
	      <td><thumbpage:order cname="手机号码" filedName="phone"/></td>
	      <td><thumbpage:order cname="座机" filedName="landline"/></td>
	      <td><thumbpage:order cname="邮箱" filedName="email"/></td>
...
<thumbpage:tools className="<%=ThumbPageConstants.CLASSNAME_DEFAULT.getCode()%>" tableName="addressList" action="/addresslist.do?method=addressList"/>
