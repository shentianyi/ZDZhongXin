...
function doLoad(){
	...
	var mailPersonDealer =$("#mailPersonDealer").val();
	changeMailPersonDealer(mailPersonDealer);
	
	var supervisoryId=$("#supervisoryId").val();
	if(supervisoryId!=null&&supervisoryId!=-1)
...
<html:hidden property="dataMailcost.approvalState" styleId="dataMailcost.approvalState"/>
...
<html:hidden property="dataMailcost.status" styleId="dataMailcost.status"/>
<table class="formTable">
	...
	<tr>
		...
		<td class="codeCol">
			<!-- <form:select property="dataMailcost.mailPersonDealer" styleId="mailPersonDealer" onchange="changeMailPersonDealer(this.value)">
				<html:option value="-1">请选择</html:option>
				<html:optionsCollection name="mailPersonDealers" label="label" value="value" />
			</form:select> -->
			<select id="mailPersonDealer"name="dataMailcost.mailPersonDealer" style="width:150px" onchange="changeMailPersonDealer(this.value)" >
	...
	<tr>
		...
		<td class="codeCol" colspan="3">
			<!-- <form:select property="dataMailcost.receiverType" styleId="receiverType" onchange="changeReceiverType(this.value)">
				<option value="-1">请选择</option>
				<html:optionsCollection name="receiverTypes" label="label" value="value" />
			</form:select> -->
			<select id="receiverType"name="dataMailcost.receiverType" style="width:150px" onchange="changeReceiverType(this.value)" >
								<option value="-1">请选择</option>
		...
		<tr id="businessOfficertr" hidden="hidden">
			...
			<td class="codeCol" colspan="3">
				<select style="width: 150px;" id="businessOfficer" name="dataMailcost.businessOfficer"  >
					<c:forEach items="${businessOfficers }" var="row">
						<option <c:if test="${dataMailcostForm.dataMailcost.businessOfficer==row.value}">selected='selected'</c:if> value="<c:out value='${row.value }'/>"><c:out value="${row.label }"/></option>
					</c:forEach>
				</select>
			</td>
		</tr>
		...
		<tr id="dealer" hidden="hidden">
			<td class="nameCol"><font color="#FF0000">*</font>经销商：</td>
			<td class="codeCol" colspan="3">
				<select style="width:150px;" id="dealerId" name="dataMailcost.dealerId"  >
					<c:forEach items="${allDealers }" var="row">
						<option <c:if test="${dataMailcostForm.dataMailcost.dealerId==row.value}">selected='selected'</c:if> value="<c:out value='${row.value }'/>"><c:out value="${row.label }"/></option>
					</c:forEach>
				</select>
			</td>
		</tr>
			...
			<td colspan="4" >
				<table width="100%">
					<tr>
						...
						<td class="codeCol">
							<select style="width: 100px;" id="supervisoryId" name="dataMailcost.supervisoryId"   onchange="changeDeliverer(this.value)">
								<c:forEach items="${supervisorys }" var="row">
									<option <c:if test="${dataMailcostForm.dataMailcost.supervisoryId==row.value}">selected='selected'</c:if> value="<c:out value='${row.value }'/>"><c:out value="${row.label }"/></option>
								</c:forEach>
							 </select>
						</td>
						<td class="nameCol">工号：</td>
	...
	<tr>
		...
		<td class="codeCol" colspan="3">
			<textarea name="dataMailcost.des" id="dataMailcost.des"><c:out value='${dataMailcostForm.dataMailcost.des }'/></textarea>
		</td>
	</tr>
