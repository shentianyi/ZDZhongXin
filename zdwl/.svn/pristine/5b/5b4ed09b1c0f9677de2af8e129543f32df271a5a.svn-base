...
<script>
	...
	//执行保存操作
	function doSave() {
		var selects = $("select[name='checkStockCarBean.actualstate']");
		var carArray = "[";
		$.each(selects,function(i,item){
			carArray += "{";
			carArray += "\"id\":" + "\""+item.id + "\""+",";
			carArray += "\"actualstate\":" + "\""+item.value + "\""+",";
			carArray += "\"remark\":" + "\""+ $("input[class='checkStockCarBean["+item.id+"].remark']").val() + "\""+",";
			if (carArray.length > 1) {
					 carArray = carArray.substr(0, carArray.length - 1);
					 carArray += "},";
					}
		})
		...
		carArray += "]";
		$("#carJson").val(carArray);
			//提交表单
		document.forms[0].submit();
		<%-- var cars="<%=request.getAttribute("checkStockCarBeansSize")%>";
		if (cars != null && cars> 0) {
		var carArray = "[";
			 for (var i = 0; i < cars; i++) {
				 carArray += "{";
				 var id=document.getElementsByName("checkStockCarBean["+i+"].id")[0].value;
				 var actualstate="0"; 
				var arr = document.getElementsByName("checkStockCarBean["+i+"].actualstate");
				for(var j=0;j<arr.length;j++){
			        if(arr[j].checked){    
			        	actualstate=arr[j].value;
			        	alert(actualstate);
			        }
				}
				carArray += "\"id\":" + "\""+id + "\""+",";
				carArray += "\"actualstate\":" + "\""+actualstate + "\""+",";
				 if (carArray.length > 1) {
					 carArray = carArray.substr(0, carArray.length - 1);
					 carArray += "},";
					}
...
<div class="pagebodyOuter">
	<div class="pagebodyInner">
		<html:form action="/supervisor/checkStockManage.do" styleId="checkStockManageForm"
			...
			<table class="formTable">
				...
				<tr>
					...
					<td class="codeCol" colspan="6">
                        ...
                        <span>辆</span>   
                      	<%-- <span style="margin-left:10%;">异常合计：</span>          
                      	<c:out value="${checkStockManageForm.checkStockBean.actual_abnormal_count}" />
                      	<span>辆</span>  --%>          
					</td>
				</tr>
				<!-- 新增 -->
				...
				<tr>
					<td class="codeCol" colspan="8">
						...
						collection="checkStockResults" styleId="checkStockBean.result" />
						...
						<logic:iterate name="checkStockManageForm" property="checkStockCarBeans"  id="checkStockCarBean" indexId="index">
							<tr class="t-tr">
								...
								<td  class="t-td" style="text-align: center;" > 
									<%-- <input type="radio"  name="checkStockCarBean[<c:out value="${index}" />].actualstate" value="1"
									<c:if test="${checkStockCarBean.actualstate==1 }">checked='checked'</c:if> /> 本库
									<input type="radio"  name="checkStockCarBean[<c:out value="${index}" />].actualstate" value="2"
									<c:if test="${checkStockCarBean.actualstate==2 }">checked='checked'</c:if> /> 二库
									<input type="radio"  name="checkStockCarBean[<c:out value="${index}" />].actualstate" value="3"
									<c:if test="${checkStockCarBean.actualstate==3 }">checked='checked'</c:if> /> 移动
									<input type="radio"  name="checkStockCarBean[<c:out value="${index}" />].actualstate" value="4"
									<c:if test="${checkStockCarBean.actualstate==4 }">checked='checked'</c:if> /> 出库 --%>
									<div class="ly-col">
										<div class="input block hidden" style="text-align: center;">
										
										 <select style="height:24px;padding: 0 auto;" class="ly-bor-none twoBankId" name="checkStockCarBean.actualstate" id="<c:out value="${checkStockCarBean.id}" />">
								...
								<td class="t-td"><input type="text"  maxlength=10 style="margin-left:34px;"  class="checkStockCarBean[<c:out value="${checkStockCarBean.id}" />].remark" name="checkStockCarBean.remark" value="<c:out value='${checkStockCarBean.remark }' />" /></td>
							</tr>
						</logic:iterate>
					</tbody>
				</table>
			...
			<table class="formTable" style="top:1260px;position:absolute">
				<tr class="formTitle">
				...
				<tr class="t-tr">
               	...
               	<tr>
               		<td colspan="2" align="center" >
                		<html:textarea style="width:80%;height:100px;"  property="checkStockBean.normal_difference" styleId="checkStockBean.normal_difference"/>
                	<td>
                	<td colspan="2" align="center" >
                		<html:textarea style="width:80%;height:100px;" property="checkStockBean.un_normal_difference" styleId="checkStockBean.un_normal_difference"/>
                	<td>
               	</tr>
               	<tr class="formTitle">
								...
								<div class="container">
									<div class="row">
										<div class="col-md-12" style="margin-left:-200px;">
											...
											<input type="hidden" id="dataType" value="text" />
										</div>
									</div>
								</div>
								</div>
				...
				<input id="carJson" type="hidden" name="carJson" value="" />
			</table>
			</html:form>
		</div>
	</div>
...
<script type="text/javascript">
 	$('#ssi-upload').ssi_uploader({url:'/supervisor/checkStockManage.do?method=addPicture&id='+$("#checkStockBean\\.id").val(),maxFileSize:5,maxNumberOfFiles:20,onUpload: function () {alert('文件上传完毕！');},onEachUpload: function (flag) {/* alert(returnResult); */},allowed:['jpg','gif','jpeg','png','bmp']}); 
