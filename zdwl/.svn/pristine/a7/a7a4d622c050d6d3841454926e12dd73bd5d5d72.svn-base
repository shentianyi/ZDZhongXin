// JavaScript Document
function initializeUploadFileTable(tableId,delFunction){
	//创建不定长TABLE对象
...
function addUploadFileRow(tableId){
	...
	newTr.childNodes[newTr.childNodes.length-1].innerHTML = "<button class=\"formButton\" onClick=\"delUploadFileRow('" + tableId + "',this.parentNode.parentNode);" + dTable.delFunctionName + "\">删除</button>";
...
function delUploadFileRow(tableId,oRow){
...
function delUploadfile(webroot,fileId,url){
