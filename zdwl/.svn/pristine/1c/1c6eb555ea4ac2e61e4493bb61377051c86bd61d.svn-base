...
//进入新增页面
function goAdd(){
	location="<url:context/>/fixedAssets.do?method=addFixedAssetsEntry";
}

//进入修改页面
function goUpd(id){
	location = "<url:context/>/fixedAssets.do?method=updFixedAssetsEntry&fixedAssets.id="+id;
}

//执行删除操作
function doDel(id){
	if(confirm("删除后数据不可恢复\n是否继续？")){
		location="<url:context/>/fixedAssets.do?method=delFixedAssets&fixedAssets.id="+id;
	}
}
...
function goImport(){
	location="<url:context/>/fixedAssets.do?method=importExcelEntry";
}
function goExt(){
//	location="<url:context/>/fixedAssets.do?method=extExcel";
	getElement("method").value = "extExcel";
	document.forms[0].submit();
	getElement("method").value = "fixedAssetsList";
}
/* 完成需求32 easyUI下拉筛选控件 */
$(function(){
...
<div class="public-main abs">
	<div class="ly-contai rel">
		...
		<input name="method" id="method" type="hidden" value="fixedAssetsList"/>
					...
					<tbody class="t-tbody hidden">
						<logic:iterate name="list" id="row" indexId="index">
							<tr class="t-tr">
								...
								<td class="t-td"><%-- <select:user userid="${row.sendee}"/> --%>
								    <select:roster sid="${row.sendee}" idtype="jgy"/>
								</td>
		...
		<div class="public-main-footer hidden abs">
			...
			<a href="javascript:goImport();" class="button btn-add fl yc">导入固定资产</a>
			<a href="javascript:goExt();" class="button btn-add fl yc">导出固定资产管理台账</a>
			<div class="public-main-footer-pagin fr">
				<thumbpage:tools className="<%=ThumbPageConstants.CLASSNAME_DEFAULT.getCode()%>" tableName="fixedAssetsList" action="/fixedAssets.do?method=fixedAssetsList"/>
			</div>
		</div>
		</html:form>
	</div>
