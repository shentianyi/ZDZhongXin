...
function doSave(){
	if(hasCheckedItem("dealerIds")){
		getElement("method").value="addDealer";
		document.forms[0].submit();
	} else{
		showMessage("请选择要添加的经销商!");
	}
}
...
function goDealer(){
	...
	location = "<url:context/>/rbac/dealerGroup.do?method=dealerEntry&dealerGroup.id="+id;
}
	...
	<div class="ly-contai clearfix">
        <div class="public-bar-tab fr hidden clearfix">
            <div class="ly-button-w fr">
                <a class="button fl" href="javascript:goDealer();">已分配经销商</a>
                <a class="button btn-sel fl" href="javascript:void(0);">分配新经销商</a>
            </div>
        </div>
	</div>
</div>
<div class="public-main abs">
	<div class="ly-contai rel">
		<html:form action="/rbac/dealerGroup" styleId="dealerGroupForm" method="post" onsubmit="return false">
			<input type="hidden" name="method" id="method" value="addDealerEntry" />
			...
			<div class="public-main-footer hidden abs">
				...
				<div class="public-main-footer-pagin fr">
					<thumbpage:tools className="<%=ThumbPageConstants.CLASSNAME_TAGBAR.getCode()%>" tableName="pageList" action="/rbac/dealerGroup.do?method=addDealerEntry"/>
				</div>
			</div>
		</html:form>
	</div>
