...
function detail(id){
	...
	//window.location.href="/ledger/dealer.do?method=dealerModeChangeLog1&query.id="+id;
}

function changeLog(id){
	window.location.href="/ledger/dealer.do?method=dealerModeChangeLog&query.id="+id;
}
...
//停用
function stopUse(id){
    window.location.href="/ledger/dealer.do?method=StopCooperationStateMarket&dealerQ.id="+id;
}
</script>
<style type="text/css">
.title td{
...
<div class="public-main abs">
	<div class="ly-contai rel">
					...
					<tbody class="t-tbody hidden">
						<logic:iterate name="list" id="row" indexId="index">
							<tr class="t-tr">
								...
								<td class="t-td fontSize14" style="font-size: 14px;"><c:out value="${row.address }"/></td>
								<td class="t-td">
								...
								<td class="t-td"><c:out value="${row.exitDate }"/></td>
								<td class="t-td">
									<a href="javascript:detail('<c:out value='${row.id}'/>');">详情</a>
									<a href="javascript:changeLog('<c:out value='${row.id }'/>');">变更日志</a>
		...
		<div class="public-main-footer hidden abs">
			<a href="javascript:goExt();" class="button btn-add fl yc">导出经销商名录表</a>
			<div id="message" style="color:red;"><c:out value='${message}'/></div>
			<div class="public-main-footer-pagin fr">
				<thumbpage:tools className="<%=ThumbPageConstants.CLASSNAME_DEFAULT.getCode()%>" tableName="dealerList" action="/ledger/dealer.do?method=findList" />
			</div>
		</div>
		</html:form>
	</div>
