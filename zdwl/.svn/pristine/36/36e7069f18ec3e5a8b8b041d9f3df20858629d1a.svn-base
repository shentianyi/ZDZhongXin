...
<script>
	...
	function doDel(id) {
		if (confirm("删除后数据不可恢复\n是否继续？")) {
			location = "<url:context/>/inspectionPlan.do?method=delete&inspectionPlan.id=" + id;
		}
	}
	...
	//已审核列表
	function goApprove() {
		location.href = "<url:context/>/inspectionPlan.do?method=findListApprove";
	}
	
	//进入详情页面
	function detail(id,planCode,status) {
		location = "<url:context/>/inspectionPlan.do?method=detailApproveno&inspectionPlan.id=" + id + 
	...
	<div class="public-main abs">
		<div class="ly-contai rel">
			<html:form action="/inspectionPlan.do" styleId="inspectionPlanForm" method="post" onsubmit="return false">
				...
				<div class="public-main-input ly-col-2  abs" style="height:165px;margin-top:20px;"margin-bottom:10px;>
					...
					<div class="ly-button-w" style="top: -12px;">
				...
				<div class="public-main-footer hidden abs">
					<div class="public-main-footer-pagin fr">
						<thumbpage:tools className="<%=ThumbPageConstants.CLASSNAME_DEFAULT.getCode()%>" 
 							tableName="list" action="/inspectionPlan.do?method=findListApproveno" />
					</div>
				</div>
			</html:form>
		</div>
