...
<table  class="listTalbe" cellpadding="0" cellspacing="0">
	<thead>
		<tr class="title">
	      ...
	      <td><thumbpage:order cname="邮寄时间" filedName="name"/></td>
	      <td><thumbpage:order cname="预计收回时间" filedName="name"/></td>
	    </tr>
	</thead>
	<tbody onMouseOver="mouseOver()" onMouseOut="mouseOut()" onClick="clickRow()">
		<tr class="listTr_a">
			...
			<td align="center"><c:out value="${send.agreement_num}"/></td>
			<td align="center"><select:timestamp timestamp="${send.agreement_date}" idtype="date"/></td>
			<td align="center"><select:timestamp timestamp="${send.back_date}" idtype="date"/></td>
		</tr>
	</tbody>  
</table>
</div>
