...
<script>
	...
	//执行查询操作
	function doQuery() {
		$("#method").val("findListDealerLedgerNo");
		if($("#one").val() > -1){
			if($("#two").val() == -1){
				alert("请选择分行！");
				return;
	...
	function detail(id,planCode,status) {
		location = "<url:context/>/inspectionPlan.do?method=detailLedgerNo&inspectionPlan.id=" + id + 
	...
	//已审核列表
	function goOld() {
		location.href = "<url:context/>/inspectionPlan.do?method=findListDealerLedger";
	}
	   //导出execl
function goExt(){
    $("#method").val("ExtFindListLedgerAudited");
    document.forms[0].submit();
}
	$(function(){
//限制超级角色操作
...
<!-- 					<a href="javascript:goAdd();" class="button btn-add fl">新增</a> -->
					<div class="public-main-footer-pagin fr">
						<thumbpage:tools className="<%=ThumbPageConstants.CLASSNAME_DEFAULT.getCode()%>" 
 							tableName="list" action="/inspectionPlan.do?method=findListDealerLedgerNo" />
					</div>
				</div>
			</html:form>
		</div>
