...
public class RepositoryRowMapper implements IImportRowMapper {
	...
	public String[] exportRow(Object obj) {
		...
		//储备库培训信息
		RepositoryTrainVO repositoryTrain=entity.getRepositoryTrain();
		...
		//储备库可派驻城市列表
		List<RepositoryDispatchCityVO> repositoryDispatchCity=entity.getRepositoryDispatchCity();
		//监管员基本信息
		...
		return (String[]) ArrayUtils.concat(result,dispatchCityStr);
	}

	@Override
	public String[] exportTitle() {
		return new String[]{"状态","原因","监管员姓名","身份证号","性别","出生日期",
				...
				,"培训内容（入职基础知识）","考核得分","考核总评（电脑水平）","考核总评（理论知识）","考核总评（沟通及其它）","备注","可派驻城市（省）1","可派驻城市（市）1","可派驻城市（区/县）1"
				,"可派驻城市（省）2","可派驻城市（市）2","可派驻城市（区/县）2","可派驻城市（省）3","可派驻城市（市）3","可派驻城市（区/县）3"
				,"可派驻城市（省）4","可派驻城市（市）4","可派驻城市（区/县）4","…"};
	}
