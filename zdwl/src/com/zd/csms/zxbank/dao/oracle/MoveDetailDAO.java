...
public class MoveDetailDAO extends DAOSupport implements IMoveDetailDAO {
	private static Log log = LogFactory.getLog(MoveDetailDAO.class);
	public MoveDetailDAO(String dataSourceName) {
		super(dataSourceName);
	}

	// 资源查询语句
	private static String select_movedetail = "SELECT ID,MDNO,MDREMOVEOUTNO,MDREMOVEINNO,MDWARENO,MDMOVENUMBER,MDCHASSISNO,MDCERTIFICATIONNO,MDCARPRICE FROM ZX_MOVEDETAIL WHERE 1=1";
	...
	@Override
	public boolean add(MoveDetail moveDetail) {
		moveDetail.setId(SqlUtil.getID(MoveDetail.class));
		log.info("移库详情对象："+moveDetail.toString());
		return super.add(moveDetail);
	}
