...
<input type="hidden" value='<c:out value="${attendanceForm.query.currentRole }" />' id="crole" />
<!-- end -->
<div class="add_nav">
...
<div class="public-main abs">
	<div class="ly-contai rel">
		<html:form action="/attendance" styleId="attendanceForm" method="post" onsubmit="return false">
			<input name="method" id="method" type="hidden" value="findAttendanceList" />
					...
					<table class="t-table" border="0" cellspacing="0" cellpadding="0">
						...
						<tbody class="t-tbody hidden">
							<logic:iterate name="list" id="row" indexId="index">
								<tr class="t-tr">
                                    <%-- <c:if test="${ ((attendanceForm.query.currentRole == 9 && attendanceForm.query.currentRole !=8) && row.state==3) || (attendanceForm.query.currentRole != 80 && row.state==3) }"> --%>
                                    <c:if test="${ (attendanceForm.query.currentRole == 9 && row.state==3 )}">
                                        <td class="t-td">
                                            <a href="javascript:doApproval('<c:out value="${row.id }"/>')" class="button btn-add fl" style="color: blue">审批不通过</a>
                                        </td>
                                    </c:if>
                                    <c:if test="${ (attendanceForm.query.currentRole != 9 && row.state!=3 )||(attendanceForm.query.currentRole == 8 && row.state==3)}">
                                    <td class="t-td"></td>
                                    </c:if>
                                    <c:if test="${ (attendanceForm.query.currentRole == 9 && (row.state!=3 || row.state==2 || row.state==0 ) )}">
                                    <td class="t-td"></td>
                                    </c:if>
                                    <!-- end -->
									...
									<%-- <input type="text" class='<c:out value="${row.id}" />'
							         ...
							         /> --%>
							         <input type="text" class='<c:out value="${row.id}" />'
							         <c:if test="${attendanceForm.query.currentRole == 9 || 
                                     (attendanceForm.query.currentRole == 8 && row.state == 1) || 
                                     (attendanceForm.query.currentRole == 8 && row.state == 3)}"> disabled='disabled'</c:if>
			...
			<div class="public-main-footer hidden abs">
				<%-- <c:if test="${(attendanceForm.query.currentRole == sc || attendanceForm.query.currentRole == 8 || attendanceForm.query.currentRole == 9) 
				&& ( attendanceForm.attendance.state == 0 || attendanceForm.attendance.state == 2 || attendanceForm.attendance.state == 3) && listSize>0}">
					<a href="javascript:doSubmit()" class="button btn-add fl">提交</a>
				</c:if> --%>
				<c:if test="${attendanceForm.query.currentRole == 8 && (attendanceForm.attendance.state == 0 || attendanceForm.attendance.state == 2 || attendanceForm.query.approvalState ==2)&& listSize>0 }">
				    <a href="javascript:doSubmit()" class="button btn-add fl">提交</a>
				</c:if>
				<c:if test="${attendanceForm.query.currentRole == 9  && attendanceForm.query.approvalState !=1 && attendanceForm.query.approvalState !=2 && listSize>0 }">
                    <a href="javascript:doSubmit()" class="button btn-add fl">提交</a>
                </c:if>
				<div class="public-main-footer-pagin fr">
					<thumbpage:tools className="<%=ThumbPageConstants.CLASSNAME_DEFAULT.getCode()%>" tableName="attendanceList" action="/attendance.do?method=findAttendanceList" />
				</div>
			</div>
		</html:form>
	</div>
