...
$(function(){
    ...
    //执行查询操作
    function doQuery(){
        $("#method").val("SignRecordCheckSpList");
...
<div class="add_nav">
    <span class="add_nav_acolor">
            ...
            &gt;
            <a class="crumbs-link" style="color:#929291;" href="#">每日考勤详情</a>
         </span>
</div>
<div class="public-main abs">
    <div class="ly-contai rel">
        <html:form action="/attendance" styleId="attendanceForm" method="post" onsubmit="return false">
            <input name="method" id="method" type="hidden" value="SignRecordCheckSpList" />
                            ...
                            <logic:iterate name="list" id="row" indexId="index">
                                <tr class="t-tr">
	                                ...
	                                <td class="t-td">
	                                  		...
	                                  		<%-- <c:if test="${row.state == 1}">审批通过</c:if>
											<c:if test="${row.state == 2}">审批不通过</c:if>
											<c:if test="${row.state == 3}">正在审批</c:if>
											<c:if test="${row.state == 0}">未提交</c:if> --%>
	                                ...
	                                <td class="t-td">
										...
										<c:if test="${role == 8 }">	
										    <c:if test="${row.days.dates == 1 && row.state == 0}">
		                                    <a href="javascript:doSubmit('<c:out value="${row.respId }"/>')" class="yc"> 提交</a>
		                                    </c:if>
		                                    <c:if test="${row.state == 0 || row.state == 2 }">
		                                    <a href="javascript:goUpdateEntry('<c:out value="${row.respId }"/>')" class="yc">修改</a>
		                                    </c:if>
	                                    </c:if>
										<c:if test="${role == 9 && row.state == 3}">
										  <a href="javascript:doApproval('<c:out value="${row.respId }"/>')" class="yc">审批</a>
										</c:if>
            ...
            <div class="public-main-footer hidden abs">
                <div class="public-main-footer-pagin fr">
                    <thumbpage:tools className="<%=ThumbPageConstants.CLASSNAME_DEFAULT.getCode()%>" tableName="SignRecordCheckSpList" action="/attendance.do?method=SignRecordCheckSpList" />
                </div>
            </div>
        </html:form>
    </div>
