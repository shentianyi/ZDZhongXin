...
function doLoad(){
	...
	//进入新增页面
	function goAdd() {
		location = "<url:context/>/market/bindDealer.do?method=preAdd";
	}
	...
	function doDel(id) {
		if (confirm("删除后数据不可恢复\n是否继续？")) {
			location = "<url:context/>/market/bindDealer.do?method=delete&bd.id=" + id;
		}
	}
	...
	function goAlreadyApproval(){
		location.href="<url:context/>/market/bindDealer.do?method=findList&query.pageType=2";
	}
	
	function doSubmit(id){
		if(confirm("确定要提交吗？")){
			location.href="<url:context/>/market/bindDealer.do?method=submit&bd.id="+id;
		}
	}
	
	function aproval_submit(id){
		if(confirm("确定要提交吗？")){
			location.href="<url:context/>/market/bindDealer.do?method=approval&bd.id="+id+"&query.approvalState=1";
		}
		
	}
	
	function approval(id){
		location.href="<url:context/>/market/bindDealer.do?method=preApproval&bd.id="+id;
	}
	...
	<div class="ly-contai rel">
		<html:form action="/market/bindDealer.do" styleId="bdForm" method="post" onsubmit="return false">
							...
							<logic:iterate name="list" id="row" indexId="index">
								<tr class="t-tr">
									...
									<td class="t-td">
										...
										<c:if test="${bdForm.query.currRole != market_commissioner&&row.approvalState==3 }">
											<c:choose>
											   <c:when test="${(row.nextApproval==supervisorymanagement_recruit
											   ||row.nextApproval==business_commissioner)&&
											   (bdForm.query.currRole == supervisorymanagement_recruit
											   ||bdForm.query.currRole == business_commissioner||bdForm.query.currRole!=80)}">
			...
			<div class="public-main-footer hidden abs">
				<c:if test="${bdForm.query.currRole == market_commissioner }">
