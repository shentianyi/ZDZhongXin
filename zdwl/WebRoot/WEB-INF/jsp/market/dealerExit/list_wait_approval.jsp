...
function doLoad(){
	...
	//进入新增页面
	function goAdd() {
		location = "<url:context/>/market/dealerExit.do?method=preAdd";
	}

	//进入修改页面
	function goUpd(id) {
		location = "<url:context/>/market/dealerExit.do?method=preUpdate&de.id=" + id;
	}

	//执行删除操作
	function doDel(id) {
		if (confirm("删除后数据不可恢复\n是否继续？")) {
			location = "<url:context/>/market/dealerExit.do?method=delete&de.id=" + id;
		}
	}
	...
	function goAlreadyApproval(){
		location.href="<url:context/>/market/dealerExit.do?method=findList&query.pageType=2";
	}
	
	function doSubmit(id){
		if(confirm("确定要提交吗？")){
			location.href="<url:context/>/market/dealerExit.do?method=submit&de.id="+id;
		}
	}
	
	function aproval_submit(id){
		if(confirm("确定要提交吗？")){
			location.href="<url:context/>/market/dealerExit.do?method=approval&de.id="+id+"&query.approvalState=1";
		}
		
	}
	
	function approval(id){
		location.href="<url:context/>/market/dealerExit.do?method=preApproval&de.id="+id;
	}
	
	function detail(id){
		location.href="<url:context/>/market/dealerExit.do?method=detailPage&de.id="+id;
	}
	$(function(){
		initTools();
	});
...
<div class="add_nav">
	<span class="add_nav_acolor">
            ...
            &gt;
            <a class="crumbs-link" style="color:#929291;" href="#">经销商撤店流转单</a>
         </span>
</div>
...
<c:set var="finance_accounting"><%=RoleConstants.FINANCE_ACCOUNTING.getCode() %></c:set>
...
<div class="public-main abs">
	<div class="ly-contai rel">
		<html:form action="/market/dealerExit.do" styleId="deForm" method="post" onsubmit="return false">
					...
					<div class="ly-row clearfix">
	                    ...
	                    <div class="ly-col fl" style="width:50%;">
	                        ...
	                        <div class="input block fl hidden" style="padding-left:20px;">
		                        <input style="width:35%;" id="createDateStart" name="query.createDateStart" type="text" value='<fmt:formatDate value="${deForm.query.createDateStart}" pattern="yyyy-MM-dd"/>' ></input>
		                        <span>至</span>
		                        <input style="width:35%;" id="createDateEnd" name="query.createDateEnd" type="text" value='<fmt:formatDate value="${deForm.query.createDateEnd}" pattern="yyyy-MM-dd"/>'></input>
	                        </div>
	                    </div>
					</div>
				</div>
							...
							<logic:iterate name="list" id="row" indexId="index">
								<tr class="t-tr">
									...
									<td class="t-td">
										...
										<c:if test="${deForm.query.currRole != market_commissioner }">
											<c:choose>
											   <c:when test="${(deForm.query.currRole == supervisorymanagement_recruit
											   ||deForm.query.currRole == business_commissioner
											   ||deForm.query.currRole == finance_accounting||deForm.query.currRole != 80)
											   &&(row.nextApproval==supervisorymanagement_recruit
											   ||row.nextApproval==business_commissioner
											   ||row.nextApproval==finance_accounting)}">
			...
			<div class="public-main-footer hidden abs">
				...
				<div class="public-main-footer-pagin fr">
					<thumbpage:tools className="<%=ThumbPageConstants.CLASSNAME_DEFAULT.getCode()%>" tableName="list" action="/market/dealerExit.do?method=findList" />
				</div>
			</div>
		</html:form>
	</div>
