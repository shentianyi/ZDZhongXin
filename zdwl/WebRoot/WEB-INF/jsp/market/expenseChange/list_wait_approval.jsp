...
function doLoad(){
	...
	//进入新增页面
	function goAdd() {
		location = "<url:context/>/market/expenseChange.do?method=preAdd";
	}

	//进入修改页面
	function goUpd(id) {
		location = "<url:context/>/market/expenseChange.do?method=preUpdate&ec.id=" + id;
	}

	//执行删除操作
	function doDel(id) {
		if (confirm("删除后数据不可恢复\n是否继续？")) {
			location = "<url:context/>/market/expenseChange.do?method=delete&ec.id=" + id;
		}
	}
	...
	function goAlreadyApproval(){
		location.href="<url:context/>/market/expenseChange.do?method=findList&query.pageType=2";
	}
	
	function doSubmit(id){
		if(confirm("确定要提交吗？")){
			location.href="<url:context/>/market/expenseChange.do?method=submit&&ec.id="+id;
		}
	}
	
	function approval(id){
		location.href="<url:context/>/market/expenseChange.do?method=preApproval&&ec.id="+id;
	}
	
	function detail(id){
		location.href="<url:context/>/market/expenseChange.do?method=detailPage&&ec.id="+id;
	}
	$(function(){
		initTools();
	});
...
<div class="add_nav">
	<span class="add_nav_acolor">
            <a class="crumbs-link" style="color:#929291;" href="#">监管费用管理</a>
            &gt;
            <a class="crumbs-link" style="color:#929291;" href="#">监管费变更流转单</a>
         </span>
</div>
...
<div class="public-main abs">
	<div class="ly-contai rel">
		<html:form action="/market/expenseChange.do" styleId="ecForm" method="post" onsubmit="return false">
					...
					<div class="ly-row clearfix">
	                    ...
	                    <div class="ly-col fl" style="width:50%;">
	                        ...
	                        <div class="input block fl hidden" style="padding-left:20px;">
		                        <input style="width:35%;" id="createDateStart" name="query.createDateStart" type="text" value='<fmt:formatDate value="${ecForm.query.createDateStart}" pattern="yyyy-MM-dd"/>' ></input>
		                        <span>至</span>
		                        <input style="width:35%;" id="createDateEnd" name="query.createDateEnd" type="text" value='<fmt:formatDate value="${ecForm.query.createDateEnd}" pattern="yyyy-MM-dd"/>'></input>
	                        </div>
	                    </div>
					</div>
				</div>
								...
								<tr class="t-tr">
									...
									<td class="t-td">
										...
										<c:if test="${row.approvalState==0 || row.approvalState==2}">
											<a class="yc" href="javascript:doSubmit('<c:out value="${row.id }"/>')">提交</a>
											<a class="yc" href="javascript:goUpd('<c:out value="${row.id }"/>')">更新</a>
											<a class="yc" href="javascript:doDel('<c:out value="${row.id }"/>')">删除</a>
										</c:if>
										<span class="yc"><c:if test="${ecForm.query.currRole != market_commissioner&&row.approvalState==3 }">
			...
			<div class="public-main-footer hidden abs">
				<c:if test="${ecForm.query.currRole == market_commissioner }">
					<a href="javascript:goAdd();" class="button btn-add fl">新增</a>
				</c:if>
				<div class="public-main-footer-pagin fr">
					<thumbpage:tools className="<%=ThumbPageConstants.CLASSNAME_DEFAULT.getCode()%>" tableName="list" action="/market/expenseChange.do?method=findList" />
				</div>
			</div>
		</html:form>
	</div>
