...
function doLoad(){
	...
	//进入新增页面
	function goAdd() {
		location = "<url:context/>/market/businessTransfer.do?method=preAdd";
	}

	//进入修改页面
	function goUpd(id) {
		location = "<url:context/>/market/businessTransfer.do?method=preUpdate&business.id=" + id;
	}

	//执行删除操作
	function doDel(id) {
		if (confirm("删除后数据不可恢复\n是否继续？")) {
			location = "<url:context/>/market/businessTransfer.do?method=delete&business.id=" + id;
		}
	}
	...
	function goAlreadyApproval(){
		location.href="<url:context/>/market/businessTransfer.do?method=findList&query.pageType=2";
	}
	
	function check(id){
		var url = "../market/businessTransferajax.do?callback=?&id="+id;
	    $.ajax({
		url:url,
		async:false,
		dataType:"jsonp",
		success:function(result){
			...
			for(var i=0;i<data.length;i++){
				var actualInDate = data[i].actualInDate;
				var trainingDate = data[i].trainingDate;
			if( actualInDate == null ||trainingDate == null){
				alert("请将信息填写完成后提交");
				return;
			}else{
				approval(id);
	...
	function checkRepositoryId(id){
		var url = "../market/businessTransferajaxzpzy.do?callback=?&id="+id;
	    $.ajax({
		url:url,
		async:false,
		dataType:"jsonp",
		success:function(result){
			var data = result.data;
			for(var i=0;i<data.length;i++){
				var RepositoryId = data[i].repositoryId;
	...
	function doSubmit(id){
		if(confirm("确定要提交吗？")){
			location.href="<url:context/>/market/businessTransfer.do?method=submit&&business.id="+id;
		}
	}
	
	function approval(id){
		if(confirm("确定要提交吗？")){
			location.href="<url:context/>/market/businessTransfer.do?method=approval&&business.id="+id;
		}
	}
	
	function detail(id){
		location.href="<url:context/>/market/businessTransfer.do?method=detailPage&&business.id="+id;
	}
	$(function(){
		initTools();
	});
...
<div class="add_nav">
	<span class="add_nav_acolor">
            ...
            &gt;
            <a class="crumbs-link" style="color:#929291;" href="#">业务流转单</a>
         </span>
</div>
...
<div class="public-main abs">
	<div class="ly-contai rel">
		<html:form action="/market/businessTransfer.do" styleId="btForm" method="post" onsubmit="return false">
				...
				<div class="ly-input-w" style="margin-top:30px;margin-bottom:-10px">
					<div class="ly-row clearfix">
	                    ...
	                    <div class="ly-col fl" style="width:50%;">
	                        ...
	                        <div class="input block fl hidden" style="padding-left:20px;">
		                        <input style="width:35%;" id="createDateStart" name="query.createDateStart" type="text" value='<fmt:formatDate value="${btForm.query.createDateStart}" pattern="yyyy-MM-dd"/>' ></input>
		                        <span>至</span>
		                        <input style="width:35%;" id="createDateEnd" name="query.createDateEnd" type="text" value='<fmt:formatDate value="${btForm.query.createDateEnd}" pattern="yyyy-MM-dd"/>'></input>
	                        </div>
	                    </div>
					</div>
				</div>
								...
								<tr class="t-tr">
									...
									<td class="t-td">
										...
										<c:if test="${row.approvalState==0 || row.approvalState==2}">
											...
											<a href="javascript:goUpd('<c:out value="${row.id }"/>')" class="yc">更新</a>
											<a href="javascript:doDel('<c:out value="${row.id }"/>')" class="yc">删除</a>
										</c:if>
										<c:if test="${(btForm.query.currRole == supervisorymanagement_recruit 
													|| btForm.query.currRole != 80 ||btForm.query.currRole == business_commissioner)&&row.approvalState==3 }">
											<c:if test="${row.nextApproval ==  btForm.query.currRole
														||btForm.query.currRole != 80 && btForm.query.currRole != 11}">
												<a href="javascript:goUpd('<c:out value="${row.id }"/>')" class="yc">审批</a>
											</c:if>
											 <c:if test="${btForm.query.currRole == business_commissioner && btForm.query.currRole != 80  && btForm.query.currRole != 11 }">
											<a href="javascript:check('<c:out value="${row.id }"/>')" class="yc">提交</a>
											</c:if>
											<c:if test="${btForm.query.currRole != business_commissioner && btForm.query.currRole != 80  && btForm.query.currRole != 11}">
											<a href="javascript:checkRepositoryId('<c:out value="${row.id }"/>')" class="yc">提交</a>
											</c:if> 
										</c:if>
									</td>
								</tr>
			...
			<div class="public-main-footer hidden abs">
				<c:if test="${btForm.query.currRole == market_commissioner }">
					<a href="javascript:goAdd();" class="button btn-add fl yc">新增</a>
				</c:if>
				<div class="public-main-footer-pagin fr">
					<thumbpage:tools className="<%=ThumbPageConstants.CLASSNAME_DEFAULT.getCode()%>" tableName="btlist" action="/market/businessTransfer.do?method=findList" />
				</div>
			</div>
		</html:form>
	</div>
