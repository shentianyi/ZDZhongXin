...
<script>
	$(function(){
		init();
		changeDealer($("#jxs").combobox('getValue'));
	});
						...
						<td class="codeCol">
							<select style="width:100%" id="jxs" name="business.dealerId" >
								<c:forEach items="${dealers }" var="row">
									<option <c:if test="${btForm.business.dealerId==row.value}">selected='selected'</c:if> value="<c:out value='${row.value }'/>"><c:out value="${row.label }"/></option>
								</c:forEach>
							</select>
						</td>
