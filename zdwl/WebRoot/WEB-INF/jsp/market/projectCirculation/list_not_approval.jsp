...
function doLoad(){
	...
	//进入新增页面
	function goAdd() {
		location = "<url:context/>/market/projectCirculationForm.do?method=preAdd";
	}

	//进入修改页面
	function goUpd(id) {
		location = "<url:context/>/market/projectCirculationForm.do?method=preUpdate&mpc.id=" + id;
	}

	//执行删除操作
	function doDel(id) {
		if (confirm("删除后数据不可恢复\n是否继续？")) {
			location = "<url:context/>/market/projectCirculationForm.do?method=delete&mpc.id=" + id;
		}
	}
	...
	function goAlreadyApproval(){
		location.href="<url:context/>/market/projectCirculationForm.do?method=findList&query.pageType=2";
	}
	
	function doSubmit(id){
		if(confirm("确定要提交吗？")){
			location.href="<url:context/>/market/projectCirculationForm.do?method=submit&query.id="+id;
		}
	}
	
	function approval(id){
		location.href="<url:context/>/market/projectCirculationForm.do?method=preApproval&query.id="+id;
	}
	
	function detail(id){
		location.href="<url:context/>/market/projectCirculationForm.do?method=deatilInfo&query.id="+id;
	}
...
<div class="add_nav">
	<span class="add_nav_acolor">
            ...
            &gt;
            <a class="crumbs-link" style="color:#929291;" href="#">项目进驻流转单</a>
         </span>
</div>
...
<div class="public-main abs">
	<div class="ly-contai rel">
		<html:form action="/market/projectCirculationForm" styleId="mpcForm" method="post" onsubmit="return false">
					...
					<div class="ly-row clearfix">
	                    ...
	                    <div class="ly-col fl" style="width:50%;">
	                        ...
	                        <div class="input block fl hidden" style="padding-left:20px;">
		                        <input style="width:35%;" id="createDateStart" name="query.createDateStart" type="text" value='<fmt:formatDate value="${mpcForm.query.createDateStart}" pattern="yyyy-MM-dd"/>' ></input>
		                        <span>至</span>
		                        <input style="width:35%;" id="createDateEnd" name="query.createDateEnd" type="text" value='<fmt:formatDate value="${mpcForm.query.createDateEnd}" pattern="yyyy-MM-dd"/>'></input>
	                        </div>
	                    </div>
							...
							<logic:iterate name="list" id="row" indexId="index">
								<tr class="t-tr">
									...
									<td class="t-td">
										...
										<c:if test="${mpcForm.query.currRole != market_commissioner&&row.approvalState==3 }">
			...
			<div class="public-main-footer hidden abs">
				<c:if test="${mpcForm.query.currRole == market_commissioner }">
					<a href="javascript:goAdd();" class="button btn-add fl yc">新增</a>
				</c:if>
				<div class="public-main-footer-pagin fr">
					<thumbpage:tools className="<%=ThumbPageConstants.CLASSNAME_DEFAULT.getCode()%>" tableName="marketProjectList" action="/market/projectCirculationForm.do?method=findList" />
				</div>
			</div>
		</html:form>
	</div>
