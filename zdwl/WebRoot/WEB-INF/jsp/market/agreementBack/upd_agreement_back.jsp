...
<body >
<div class="title">修改协议邮寄</div>
<div class="pagebodyOuter">
	<div class="pagebodyInner">
	
<html:form action="/agreementBack" styleId="agreementBackForm" method="post" onsubmit="return false">
<input type="hidden" name="method" id="method" value="updAgreementBack"/>
<input type="hidden" name="agreementBack.id" value="<c:out value='${agreementBackForm.agreementBack.id }'/>"/>
<input type="hidden" name="agreementBack.isback" value="<c:out value='${agreementBackForm.agreementBack.isback }'/>"/>
<table class="formTable">
	<tr>
		<td class="nameCol">经销商：</td>
		<td class="codeCol">
			<select style="width: 100%" id="jxs"
				name="agreementBack.distribid">
					<c:forEach items="${dealersOptions }" var="row">
						<option
							<c:if test="${agreementBackForm.agreementBack.distribid==row.value}">selected='selected'</c:if>
