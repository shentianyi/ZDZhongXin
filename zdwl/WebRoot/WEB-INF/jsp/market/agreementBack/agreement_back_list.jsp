...
//进入新增页面
function goAdd(){
	location="<url:context/>/agreementBack.do?method=addAgreementBackEntry";
}

//进入修改页面
function goUpd(id){
	location = "<url:context/>/agreementBack.do?method=updAgreementBackEntry&agreementBack.id="+id;
}
function goBack(id,address){
	if(address==null||address==""){
		alert("请填写存放地址");
		return false;
	}
	if(confirm("确认收回？")){
		location = "<url:context/>/agreementBack.do?method=goBack&agreementBack.id="+id;
	}
}
...
function doDel(id){
	if(confirm("删除后数据不可恢复\n是否继续？")){
		location="<url:context/>/agreementBack.do?method=delAgreementBack&agreementBack.id="+id;
	}
}
...
<div class="public-bar hidden">
	<div class="ly-contai clearfix">
		<div class="public-bar-crumbs fl hidden">
            ...
            > 
            <a class="crumbs-link" href="#">协议管理台账</a>
        </div>
	</div>
</div>
<div class="public-main abs">
	<div class="ly-contai rel">
		...
		<input name="method" id="method" type="hidden" value="agreementBackList"/>
					...
					<tbody class="t-tbody hidden">
						<logic:iterate name="list" id="row" indexId="index">
							<tr class="t-tr">
								...
								<td class="t-td yc">
									<c:if test="${row.isback == 1}">
										<a href="javascript:goBack('<c:out value='${row.id}'/>','<c:out value='${row.deposit_address }'/>');" >收回</a>&nbsp;
									</c:if>
		...
		<div class="public-main-footer hidden abs">
			...
			<div class="public-main-footer-pagin fr">
				<thumbpage:tools className="<%=ThumbPageConstants.CLASSNAME_DEFAULT.getCode()%>" tableName="agreementBackList" action="/agreementBack.do?method=agreementBackList"/>
			</div>
		</div>
		</html:form>
	</div>
