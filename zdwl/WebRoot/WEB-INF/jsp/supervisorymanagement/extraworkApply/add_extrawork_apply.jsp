...
function initTools(){
	...
	/* var c1 = $('#startTime').datebox('calendar');
	c1.calendar({
	...
	/* var c2 = $('#endTime').datebox('calendar');
	c2.calendar({
		validator: function(date){
			 if(date>=now){
				return true;
...
<input type="hidden" name="method" id="method" value="addExtraworkApply"/>
			...
			<table class="formTable2" style="width: 100%">
				...
				<c:if test="${currentDealerList!=null}">
					<logic:iterate name="currentDealerList" id="row" indexId="index" >
						<tr>
							<td class="nameC" style="width:15%">经销商<c:out value="${index+1}" />：</td>
							<td class="codeC" style="width:15%">
								<c:out value='${row.dealerName}' />
							</td>
							<td class="nameC" style="width:15%">金融机构<c:out value="${index+1}" />：</td>
							<td class="codeC" style="width:25%">
								<c:out value='${row.bankName}' />
							</td>
							<td class="nameC" style="width:15%">品牌<c:out value="${index+1}" />：</td>
							<td class="codeC" style="width:15%">
				...
				<tr>
					<td class="nameC"><font color="#FF0000">*</font>开始时间：</td>
					<td class="codeC" >
						<input style="width: 80%" id="startTime" name="extraworkApply.startTime" type="text"/>
					</td>
					<td class="nameC"><font color="#FF0000">*</font>结束时间：</td>
					<td class="codeC" >
						<input style="width: 80%" id="endTime" name="extraworkApply.endTime" type="text"/>
					</td>
					<td class="nameC" ><font color="#FF0000">*</font>加班天数：</td>
					<td class="codeC" >
						<input type="text" id="extraWorkDays" name="extraworkApply.extraWorkDays" class="easyui-numberbox" 
								data-options="min:0,precision:1"/>
					</td>
				</tr>
				<tr>
					<%-- <td class="nameC"><font color="#FF0000">*</font>审批部门：</td>
					<td class="codeC">
					...
					<td class="nameC"><font color="#FF0000">*</font>加班事由：</td>
					<td class="codeC" colspan="5">
						<html:textarea style="width:90%" property="extraworkApply.reason" styleId="reason"/>
					</td>
				</tr>
				<tr>
					<td colspan="6"class="nameC"></td>
				</tr>
			</table>
		</td>	
	</tr>
