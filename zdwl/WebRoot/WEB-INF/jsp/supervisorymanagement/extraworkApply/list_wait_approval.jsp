...
$(function(){
	...
	//进入新增页面
	function goAdd() {
		location="<url:context/>/extraworkApply.do?method=addExtraworkApplyEntry";
	}
	...
	function goAlreadyApproval(){
		location.href="<url:context/>/extraworkApply.do?method=findPageList&query.pageType=2";
	}
	...
	<div class="ly-contai rel">
		<html:form action="/extraworkApply" styleId="extraworkApplyForm" method="post" onsubmit="return false">
							...
							<logic:iterate name="list" id="row" indexId="index">
								<tr class="t-tr">
									...
									<td class="t-td">
										...
										<c:if test="${row.approvalState==0 && row.status==0  && extraworkApplyForm.query.currentRole == 10}">
										...
										<c:if test="${extraworkApplyForm.query.currentRole == 14 && row.approvalState==3 }">
											<a href="javascript:approval('<c:out value="${row.id }"/>')">审批</a>
										</c:if>
										<c:if test="${extraworkApplyForm.query.currentRole == 16 && row.approvalState==3 }">
											<a href="javascript:approval('<c:out value="${row.id }"/>')">审批</a>
										</c:if>
										<c:if test="${extraworkApplyForm.query.currentRole == 8 && row.approvalState==3 }">
                                            <a href="javascript:approval('<c:out value="${row.id }"/>')">审批</a>
                                        </c:if>
                                        <c:if test="${extraworkApplyForm.query.currentRole == 9 && row.approvalState==3 }">
                                            <a href="javascript:approval('<c:out value="${row.id }"/>')">审批</a>
                                        </c:if>
                                        <c:if test="${extraworkApplyForm.query.currentRole == 28 && row.approvalState==3 }">
                                            <a href="javascript:approval('<c:out value="${row.id }"/>')">审批</a>
                                        </c:if>
										<%-- <c:if test="${extraworkApplyForm.query.currentRole == 18 && row.approvalState==3 }">
											<a href="javascript:approval('<c:out value="${row.id }"/>')">审批</a>
										</c:if>
										<c:if test="${extraworkApplyForm.query.currentRole == 20 && row.approvalState==3 }">
											<a href="javascript:approval('<c:out value="${row.id }"/>')">审批</a>
										</c:if>
										<c:if test="${extraworkApplyForm.query.currentRole == 11 && row.approvalState==3 }">
											<a href="javascript:approval('<c:out value="${row.id }"/>')">审批</a>
										</c:if>
										<c:if test="${extraworkApplyForm.query.currentRole == 13 && row.approvalState==3 }">
											<a href="javascript:approval('<c:out value="${row.id }"/>')">审批</a>
										</c:if> --%>
										<%-- <c:if test="${extraworkApplyForm.query.currentRole == 23 && row.approvalState==3 }">
											<a href="javascript:approval('<c:out value="${row.id }"/>')">审批</a>
										</c:if> --%>
									</td>
								</tr>
			...
			<div class="public-main-footer hidden abs">
				<c:if test="${extraworkApplyForm.query.currentRole == sc || extraworkApplyForm.query.currentRole == 10}">
