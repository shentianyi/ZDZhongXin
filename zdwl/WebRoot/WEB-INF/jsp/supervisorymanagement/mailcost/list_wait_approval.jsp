...
<script>
	...
	//进入新增页面//
	function goAdd() {
		location="<url:context/>/mailcost.do?method=addMailcostEntry";
	}

	//执行删除操作
	function doDel(id) {
		if (confirm("删除后数据不可恢复\n是否继续？")) {
			location = "<url:context/>/mailcost.do?method=delMailcost&mailcost.id=" + id;
		}
	}
	
	//已审批列表
	function goAlreadyApproval(){
		location.href="<url:context/>/mailcost.do?method=findList&mailcostquery.pageType=2";
	}
	
	function doSubmit(id){
		if(confirm("确定要提交吗？")){
			location.href="<url:context/>/mailcost.do?method=submit&mailcost.id="+id;
		}
	}
	
	function approval(id){
		location.href="<url:context/>/mailcost.do?method=preApproval&mailcost.id="+id;
	}
	
	function detail(id){
		location.href="<url:context/>/mailcost.do?method=detailPage&mailcost.id="+id;
	}
	
	//进入修改页面
	function doUpd(id){
		location = "<url:context/>/mailcost.do?method=updMailcostEntry&mailcost.id="+id;
	}
	//执行查询操作
	function doQuery(){
...
<div class="add_nav">
	...
	<c:set var="sc"><%=RoleConstants.SUPERVISORYMANAGEMENT_COMPREHENSIVE.getCode() %></c:set>
	<c:set var="SUPERVISORY"><%=RoleConstants.SUPERVISORY.getCode() %></c:set>
<c:if test="${mailcostForm.mailcostquery.currRole !=  SUPERVISORY}">
	...
	<div class="ly-contai rel">
		<html:form action="/mailcost" styleId="mailcostForm" method="post" onsubmit="return false">
				...
				<div class="ly-row clearfix">
					 <div class="ly-col fl">
	                   ...
	                   <div class="ly-col fl">
						...
						<!-- 	<form:select styleClass="ly-bor-none" property="mailcostquery.mailperson" styleId="mailperson" >
								<html:option value="-1">请选择</html:option>
								<html:optionsCollection name="superviseOptions" label="label" value="value" />
							</form:select> -->
							<select id="mailperson" style="width:100px;" name="mailcostquery.mailperson" >
							...
							<logic:iterate name="list" id="row" indexId="index">
								<tr class="t-tr">
									...
									<td class="t-td">
										...
										<c:if test="${mailcostForm.mailcostquery.currRole != sc && row.approvalState==3 }">
			...
			<div class="public-main-footer hidden abs">
				<c:if test="${mailcostForm.mailcostquery.currRole == sc }">
					<a href="javascript:goAdd();" class="button btn-add fl yc">新增</a>
				</c:if>
				<div class="public-main-footer-pagin fr">
					<thumbpage:tools className="<%=ThumbPageConstants.CLASSNAME_DEFAULT.getCode()%>" tableName="findList" action="/mailcost.do?method=findList" />
				</div>
			</div>
		</html:form>
	</div>
