...
$(function(){
	...
	//进入新增页面
	function goAdd() {
		location="<url:context/>/resignApply.do?method=addResignEntry";
	}

	//执行删除操作
	function doDel(id) {
		if (confirm("删除后数据不可恢复\n是否继续？")) {
			location = "<url:context/>/resignApply.do?method=deleteResignApply&resignApply.id=" + id;
		}
	}
	
	//已审批列表
	function goAlreadyApproval(){
		location.href="<url:context/>/resignApply.do?method=findPageList&query.pageType=2";
	}
	...
	function approval(id){
		location.href="<url:context/>/resignApply.do?method=preApproval&resignApply.id="+id;
	}
	...
	//进入修改页面
	function doUpd(id){
		location = "<url:context/>/resignApply.do?method=updateResignApplyEntry&resignApply.id="+id;
	}
	//执行查询操作
	function doQuery(){
	...
	<div class="ly-contai rel">
		<html:form action="/resignApply" styleId="resignApplyForm" method="post" onsubmit="return false">
							...
							<logic:iterate name="list" id="row" indexId="index">
								<tr class="t-tr">
									...
									<td class="t-td">
										...
										<%-- <c:if test="${row.approvalState==0 && row.status==0  && resignApplyForm.query.currentRole == 10}">
										...
										<c:if test="${resignApplyForm.query.currentRole == 14 && row.approvalState==3 }">
											<a href="javascript:approval('<c:out value="${row.id }"/>')">审批</a>
										</c:if>
										<c:if test="${resignApplyForm.query.currentRole == 16 && row.approvalState==3 }">
                                            <a href="javascript:approval('<c:out value="${row.id }"/>')">审批</a>
                                        </c:if>
										<c:if test="${resignApplyForm.query.currentRole == 4 && row.approvalState==3 }">
											<a href="javascript:approval('<c:out value="${row.id }"/>')">审批</a>
										</c:if>
										<c:if test="${resignApplyForm.query.currentRole == 9 && row.approvalState==3 }">
											<a href="javascript:approval('<c:out value="${row.id }"/>')">审批</a>
										</c:if>
										<%-- <c:if test="${resignApplyForm.query.currentRole == 80 && row.approvalState==3 }">
											<a href="javascript:approval('<c:out value="${row.id }"/>')">审批</a>
										</c:if> --%>
									</td>
								</tr>
			...
			<div class="public-main-footer hidden abs">
			    <c:if test="${resignApplyForm.query.currentRole == sc || resignApplyForm.query.currentRole == 10}">
