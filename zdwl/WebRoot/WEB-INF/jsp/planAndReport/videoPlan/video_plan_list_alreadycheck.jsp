...
<script>
	...
	//执行查询操作
	function doQuery() {
		$("#method").val("findListAlreadyCheck");
		if($("#one").val() > -1){
			if($("#two").val() == -1){
				alert("请选择分行！");
				return;
	...
	function detail(id,planCode,status) {
		location = "<url:context/>/videoPlan.do?method=detailAlreadyCheck&videoPlan.id=" + id + 
	...
	//已审核列表
	function goOld() {
		location.href = "<url:context/>/videoPlan.do?method=findList";
	}
		...
		<div class="ly-contai clearfix">
			<div class="public-bar-tab fr hidden clearfix">
				<div class="ly-button-w fr">
					<a class="button fl" href="javascript:goOld();">未审核</a> 
					<a class="button btn-sel fl" href="javascript:void(0);">已审核</a>
				</div>
			</div>
		</div>
	</div>
					...
					<div class="ly-input-w" style="margin-top:30px;margin-bottom:-10px">
						<div class="ly-row clearfix">
							...
							<div class="ly-col fl">
								...
								<div class="input block fl hidden">
									<select id="provinceId" name="query.provinceId" class=" ly-bor-none provinceId" style="height:30px; " onchange="changeProvince(this.value,$('#cityId'))">
										<option value="-1">请选择...</option>
									</select>
								</div>
							</div>
...
<%-- 										<td class="t-td"><c:out value="${row.unCheckPassCause}" /></td> --%>
										<td class="t-td">
											<a id="detailbt" href="javascript:detail('<c:out value="${row.id }"/>',
											'<c:out value="${row.planCode }"/>','<c:out value="${row.status }"/>')">详情</a>
										</td>
									</tr>
								</logic:iterate>
							</tbody>
...
<!-- 					<a href="javascript:goAdd();" class="button btn-add fl">新增</a> -->
					<div class="public-main-footer-pagin fr">
						<thumbpage:tools className="<%=ThumbPageConstants.CLASSNAME_DEFAULT.getCode()%>" 
 							tableName="list" action="/videoPlan.do?method=findListAlreadyCheck" />
					</div>
				</div>
			</html:form>
		</div>
