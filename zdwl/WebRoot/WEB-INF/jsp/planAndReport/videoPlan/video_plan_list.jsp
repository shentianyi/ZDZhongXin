...
<script>
	...
	function goAlreadyCheck() {
		location.href = "<url:context/>/videoPlan.do?method=findListAlreadyCheck";
	}
	
	//进入详情页面
	function detail(id,planCode,status) {
		location = "<url:context/>/videoPlan.do?method=detail&videoPlan.id=" + id + 
	...
	//进入修改页面
	function goUpd(id,planCode,status) {
		...
		<div class="ly-contai clearfix">
			<div class="public-bar-tab fr hidden clearfix">
				<div class="ly-button-w fr">
					...
					<a class="button fl" href="javascript:goAlreadyCheck();">已审核</a>
				</div>
			</div>
		</div>
	</div>
					...
					<div class="ly-input-w" style="margin-top:30px;margin-bottom:-10px">
						<div class="ly-row clearfix">
							<div class="ly-col fl">
								...
								<div class="input block fl hidden">
									<select id="one" name="query.oneBankId" class="oneBankId ly-bor-none"  style="height:30px;margin-top:2px;">
							...
							<div class="ly-col fl">
								...
								<div class="input block fl hidden">
									<select id="two" name="query.twoBankId" class="twoBankId ly-bor-none" style="height:30px;margin-top:2px;">
										<option value="-1">请选择...</option>
									</select>
								</div>
							</div>
							<div class="ly-col fl">
								...
								<div class="input block fl hidden">
									<div class="ly-sel-w">
										<select id="three" name="query.threeBankId" class="threeBankId ly-bor-none" style="height:30px;margin-top:-2px;">
											<option value="-1">请选择...</option>
										</select>
									</div>
								</div>
							...
							<div class="ly-col fl">
								...
								<div class="input block fl hidden ">
									<div class="ly-sel-w">
										<select id="provinceId" name="query.provinceId" class=" ly-bor-none provinceId" style="height:30px;margin-top:-2px;" onchange="changeProvince(this.value,$('#cityId'))">
											<option value="-1">请选择...</option>
										</select>
									</div>
								</div>
						...
						<div class="ly-row clearfix">
							<div class="ly-col fl">
								...
								<div class="input block fl hidden">
									<select id="cityId" name="query.cityId" class="cityId ly-bor-none" style="height:30px;margin-top:2px;">
										<option value="-1">请选择...</option>
									</select>
								</div>
							</div>
								...
								<logic:iterate name="list" id="row" indexId="index">
									<tr class="t-tr">
										...
										<td class="t-td">
											...
											<c:if test="${row.status == 1 || row.status == 4}">
												<a id="submitbt" href="javascript:doSubmit('<c:out value="${row.id }"/>','<c:out value="${row.planCode }"/>')">提交</a>
												<a id="updatbt" href="javascript:goUpd('<c:out value="${row.id }"/>','<c:out value="${row.planCode }"/>'
												,'<c:out value="${row.status }"/>')">更新</a>
											</c:if>
											<c:if test="${row.status == 1}">
												<a id="deletebt" href="javascript:doDel('<c:out value="${row.id }"/>','<c:out value="${row.planCode }"/>')">删除</a>
											</c:if>
										</td>
									</tr>
								</logic:iterate>
				...
				<div class="public-main-footer hidden abs">
					...
					<div class="public-main-footer-pagin fr">
						<thumbpage:tools className="<%=ThumbPageConstants.CLASSNAME_DEFAULT.getCode()%>" 
 							tableName="list" action="/videoPlan.do?method=findList" />
					</div>
				</div>
			</html:form>
		</div>
