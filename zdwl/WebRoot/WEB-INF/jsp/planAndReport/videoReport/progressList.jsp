							  ...
							  <c:if test="${!empty list}">
								<logic:iterate name="list" id="row" indexId="index">
									<tr class="t-tr">
										...
										<td class="t-td">
										    <c:if test="${row.editStatus==0}">未编辑</c:if>
											<c:if test="${row.editStatus==1}">已编辑</c:if> 
									     ...
									     <td class="t-td yc">
									      <a href="javascript:goUpd('<c:out value="${row.id }"/>')">编辑</a>
									    </td>
				...
				<div class="public-main-footer hidden abs">
					...
					<div class="public-main-footer-pagin fr">
						<thumbpage:tools
							...
							tableName="pagelist" action="/videoReport.do?method=findProgressList" />
					</div>
				</div>
			</html:form>
		</div>
