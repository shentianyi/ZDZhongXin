...
function doSave(){
	...
	var clientId = getElement("user.client_id");

	if(clientType.value!=null && clientType.value == "-1"){
		showMessage("请选择账户类型");
		return false;
	}
...
<div class="add_nav">
	<span class="add_nav_acolor">
            ...
            <a class="crumbs-link" style="color:#929291;" href="#">账户管理</a>
             &gt;
            <a class="crumbs-link" style="color:#929291;" href="#">新增账户</a>
         </span>
</div>
...
<table class="formTable">
	...
	<tr>
		...
		<td class="codeCol"><html:password property="user.password" styleId="user.password" value="0000aaaa"/></td>
		<td class="nameCol"><font color="#FF0000">*</font>确认登录密码：</td>
		<td class="codeCol"><input type="password" name="confirmPassword" id="confirmPassword" value="0000aaaa"></td>
	</tr>
	<tr>
		...
		<td class="codeCol">
			...
			<select id="user.client_type" name="user.client_type" onchange="cClientType()" class="ly-bor-none" >
	                  <c:forEach items="${clientTypeOptions }" var="row">
	                    	<option <c:if test="${row.value==userForm.user.client_type}">selected='selected'</c:if> value="<c:out value='${row.value}'/>"><c:out value="${row.label }"/></option>
	                  </c:forEach>
	        </select>
		</td>
...
注：<br>
用户类型分为：<br>
1、机构成员，属于机构下的业务数据，不具有系统访问权限<br>
2、系统用户，拥有访问系统的权限
</p>
-->
</body>
