...
function doDel(){
	//检查是否选择了要删除的资源
	if(hasCheckedItem("resourceIds")){
		if(confirm("确定删除?")){
			//将表单提交至addResourceRole方法
			getElement("method").value="delRoleResource";
			document.forms[0].submit();
		}
	} else{
		showMessage("请选择要删除的资源!");
	}
}

//进入角色下未分配资源页面
function goResourceWithoutRole(){
	location = "<url:context/>/rbac/resource.do?method=searchResourceWithoutRole&roleId=<c:out value="${roleId}"/>&<thumbpage:setQueryFlag tableName="searchResourceWithoutRole" out="true"/>";
}
	...
	<div class="ly-contai clearfix">
        <div class="public-bar-tab fr hidden clearfix">
            <div class="ly-button-w fr">
                <a class="button btn-sel fl" href="javascript:void(0);">已分配资源</a>
                <a class="button fl" href="javascript:goResourceWithoutRole();">分配新资源</a>
            </div>
        </div>
	</div>
</div>
<div class="public-main abs">
	<div class="ly-contai rel">
		<html:form action="/rbac/resource" styleId="resourceForm" method="post" onsubmit="return false">
			<input type="hidden" name="method" id="method" value="resourceListWithRole" />
					...
					<table class="t-table" border="0" cellspacing="0" cellpadding="0">
						<thead class="t-thead">
							<tr class="t-tr">
								<th class="t-th">
									...
									<input type="checkbox" name="checkAllValue" id="checkAllValue" class="checkbox_title" onClick="checkAll('resourceIds')">
								</th>
								<th class="t-th">序号</th>
								<th class="t-th">目录</th>
								<th class="t-th">名称</th>
						...
						<tbody class="t-tbody hidden">
							<logic:iterate name="list" id="row" indexId="index">
								<tr class="t-tr">
									<td class="t-td">
										<input type="checkbox" id="resourceIds" name="resourceIds" value="<c:out value='${row.id}'/>">
									</td>
									<td class="t-td"><c:out value="${index+1}"/></td>
									<td class="t-td"><select:menuName bid="${row.parent_id}"/></td>
									<td class="t-td"><c:out value="${row.resource_name}"/></td>
									<td class="t-td"><c:out value="${row.resource_url}"/></td>
									<td class="t-td">
			...
			<div class="public-main-footer hidden abs">
				...
				<div class="public-main-footer-pagin fr">
					<thumbpage:tools className="<%=ThumbPageConstants.CLASSNAME_TAGBAR.getCode()%>" tableName="resourceListWithRole" action="/rbac/resource.do?method=resourceListWithRole"/>
				</div>
			</div>
		</html:form>
	</div>
