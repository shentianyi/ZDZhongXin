...
<script>
	$(function(){
		...
		var whcustNo=$("#warehouse_custNo").val();
		var choose=$("#queryType").val();
		$("#whcustNo option[value='"+whcustNo+"']").attr("selected","selected");
	    ...
	    comboboxSetId("#whcustNo");
	});
	
	function doQuery() {
		/* if ($('#choose').val() == "2" && $("#whcustNo").find("option:selected").text()=="请选择...") {
			alert("ECIF客户号不能为空");
			return;
		}  */
	...
	<div class="public-bar hidden">
		<div class="ly-contai clearfix">
			<div class="public-bar-crumbs fl hidden">
				<a class="crumbs-link" href="#">中信银行接口</a> &gt; <a class="crumbs-link" href="#">仓库信息查询</a>
			</div>
		</div>
	</div>
						...
						<div class="ly-row clearfix">
							<%-- <div class="ly-col fl">
								...
								<div class="input block fl hidden">
									<input class="ly-bor-none" type="text" id="custNo" name="warehouse.custNo" value="${warehouse.custNo}" maxlength="20" />
								</div>
							</div> --%>
							...
							<div class="ly-col fl">
								...
								<div class="input block fl hidden">
									<div class="ly-sel-w">
										<select id="whcustNo" name="warehouse.custNo" style="width: 85%;">
											<option value="">请选择...</option>
											<c:forEach items="${draftOptions}" var="row">
												<option <c:if test="${row.label==warehouse.custNo}"></c:if> value="<c:out value='${row.value }'/>"><c:out value="${row.label }" /></option>
											</c:forEach>
										</select> <input type="hidden" id="warehouse_custNo" value="${warehouse.custNo}" />
									</div>
								</div>
							</div>
							<div class="ly-col fl">
								...
								<div class="input block fl hidden">
									<input class="ly-bor-none" type="text" id="whName" name="warehouse.whName" value="${warehouse.whName}" maxlength="120" />
								</div>
							</div>
									...
									<c:if test="${not empty list }">
										<logic:iterate name="list" id="row" indexId="index">
											<tr class="t-tr">
												...
												<td class="t-td"><a href="ZXinterface.do?method=warehouseUpdateShow&custNo=${row.custNo}&whCode=${row.whCode}">编辑</a></td>
											</tr>
										</logic:iterate>
									</c:if>
								</tbody>
