...
<script type="text/javascript" language="javascript">
	$(function(){
		...
		var rpNo=$("#removepledge_rpNo").val();
		$("#rpNo option[value='"+rpNo+"']").attr("selected","selected");
	...
	<div class="public-bar hidden">
		<div class="ly-contai clearfix">
			<div class="public-bar-crumbs fl hidden">
				<a class="crumbs-link" href="/ZXBank">中信银行接口</a> &gt; <a class="crumbs-link" href="#">解除质押通知</a>
			</div>
		</div>
	</div>
			...
			<html:form action="/ZXinterface.do" styleId="removeForm" method="post" onsubmit="return false">
						...
						<div class="ly-row clearfix">
							<div class="ly-col fl">
								...
								<div class="input block fl hidden">
									<div class="ly-sel-w">
										<select id="rpNo" name="removepledge.rpNo" style="width: 85%;">
											<option value="">请选择...</option>
											<c:forEach items="${draftOptions}" var="row">
												<option <c:if test="${row.label==removepledge.rpNo}"></c:if> value="<c:out value='${row.value }'/>"><c:out value="${row.label }" /></option>
											</c:forEach>
										</select> <input type="hidden" id="removepledge_rpNo" value="${removepledge.rpNo}" />
									</div>
								</div>
							</div>
									...
									<c:if test="${not empty list }">
										<logic:iterate name="list" id="row" indexId="index">
											<tr class="t-tr">
												...
												<td class="t-td"><a href="ZXinterface.do?method=removepledgedetail&rdno=<c:out value='${row.rpNo }'/>">详情</a></td>
											</tr>
										</logic:iterate>
									</c:if>
								</tbody>
				...
				<div class="public-main-footer hidden abs">
					<div class="public-main-footer-pagin fr">
						<thumbpage:tools className="<%=ThumbPageConstants.CLASSNAME_DEFAULT.getCode()%>" tableName="RemovePledge" action="ZXinterface.do?method=removepledge" />
					</div>
				</div>
			</html:form>
		</div>
