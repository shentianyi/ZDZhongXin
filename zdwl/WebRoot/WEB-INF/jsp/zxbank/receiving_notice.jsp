...
<script>
	$(function(){
		...
		var nyNo=$("#receivingnotice_nyNo").val();
		$("#nyNo option[value='"+nyNo+"']").attr("selected","selected");
		comboboxSetId("#nyNo");
	})
	function doQuery() {
	...
	<div class="public-bar hidden">
		<div class="ly-contai clearfix">
			<div class="public-bar-crumbs fl hidden">
				<a class="crumbs-link" href="#">中信银行接口</a> &gt; <a class="crumbs-link" href="#">收货通知</a>
			</div>
		</div>
	</div>
	<div class="public-main abs">
		<div class="ly-contai rel">
			<html:form action="/ZXinterface.do" styleId="receForm" method="post" onsubmit="return false">
						...
						<div class="ly-row clearfix">
							<div class="ly-col fl">
								...
								<div class="input block fl hidden">
									<div class="ly-sel-w">
										<select id="nyNo" name="receivingnotice.nyNo" style="width: 85%;">
											<option value="">请选择...</option>
											<c:forEach items="${draftOptions}" var="row">
												<option <c:if test="${row.label==receivingnotice.nyNo}"></c:if> value="<c:out value='${row.value }'/>"><c:out value="${row.label }" /></option>
											</c:forEach>
										</select> <input type="hidden" id="receivingnotice_nyNo" value="${receivingnotice.nyNo}" />
									</div>
								</div>
							</div>
							...
							<div class="ly-col fl">
								...
								<div class="input block fl hidden">
									<input class="ly-bor-none" type="text" id="nyLonentname" name="receivingnotice.nyLonentname" value="${receivingnotice.nyLonentname }" maxlength="120" />
								</div>
							</div>
						</div>
					</div>
							...
							<tbody class="t-tbody hidden">
								<logic:iterate name="list" id="row" indexId="index">
									<tr class="t-tr">
										...
										<td class="t-td"><select:timestamp timestamp="${row.nyNtcdate}" idtype="ss" /></td>
										<td class="t-td"><select:timestamp timestamp="${row.nyCsndate}" idtype="ss" /></td>
										<td class="t-td"><select:timestamp timestamp="${row.nyEta}" idtype="ss" /></td>
										<td class="t-td"><c:out value="${row.nyEpa }" /></td>
										<td class="t-td"><c:out value="${row.nyRemark }" /></td>
										<td class="t-td"><c:out value="${row.nyTotnum}" /></td>
										<td class="t-td"><select:timestamp timestamp="${row.nyCreatedate}" idtype="ss" /></td>
										<td class="t-td"><select:timestamp timestamp="${row.nyUpdatedate}" idtype="ss" /></td>
										<td class="t-td"><a href="ZXinterface.do?method=receivingdetail&nyNo=${row.nyNo}">详情</a></td>
									</tr>
								</logic:iterate>
							</tbody>
						</table>
				...
				<div class="public-main-footer hidden abs">
					<div class="public-main-footer-pagin fr">
						<c:if test="${not empty list }">
							<thumbpage:tools className="<%=ThumbPageConstants.CLASSNAME_DEFAULT.getCode()%>" tableName="ReceivingNotice" action="ZXinterface.do?method=receivingnotice" />
						</c:if>
					</div>
				</div>
			</html:form>
