...
<script>
	$(function(){
		...
		var hostno=$("#agreement_hostno").val();
		var choose=$("#queryType").val();
		$("#hostno option[value='"+hostno+"']").attr("selected","selected");
		...
		comboboxSetId("#hostno");
	});
	function doQuery() {
		 /* if ($('#choose').val() == "2" && $("#hostno").find("option:selected").text() == "请选择...") {
			alert("ECIF客户号不能为空");
			return;
		} */
	...
	<div class="public-bar hidden">
		<div class="ly-contai clearfix">
			<div class="public-bar-crumbs fl hidden">
				<a class="crumbs-link" href="#">中信银行接口</a> &gt; <a class="crumbs-link" href="#">监管协议查询</a>
			</div>
		</div>
	</div>
			...
			<html:form action="/ZXinterface.do" styleId="agrForm" method="post" onsubmit="return false">
					...
					<div class="ly-input-w">
						<div class="ly-row clearfix">
							...
							<div class="ly-col fl">
								...
								<div class="input block fl hidden">
									<div class="ly-sel-w">
										<select id="hostno" name="agreement.hostno" style="width: 85%;">
											<option value="">请选择...</option>
											<c:forEach items="${draftOptions}" var="row">
												<option <c:if test="${row.label==agreement.hostno}"></c:if> value="<c:out value='${row.value }'/>"><c:out value="${row.label }" /></option>
											</c:forEach>
										</select> <input type="hidden" id="agreement_hostno" value="${agreement.hostno}" />
									</div>
								</div>
							</div>
									...
									<c:if test="${not empty list }">
										<logic:iterate name="list" id="row" indexId="index">
											<tr class="t-tr">
												...
												<td class="t-td"><c:if test="${row.isauth=='00'}">未开通</c:if> <c:if test="${row.isauth=='01'}">开通</c:if></td>
												<td class="t-td"><c:if test="${row.ismv=='00'}">不允许</c:if> <c:if test="${row.ismv=='01'}">允许</c:if></td>
												<td class="t-td"><c:if test="${row.agtstt=='01'}">生效</c:if> <c:if test="${row.agtstt=='02' }">失效</c:if></td>
				...
				<div class="public-main-footer hidden abs">
					<div class="public-main-footer-pagin fr">
						<c:if test="${not empty list }">
							<thumbpage:tools className="<%=ThumbPageConstants.CLASSNAME_DEFAULT.getCode()%>" tableName="Agreement" action="ZXinterface.do?method=agreement" />
						</c:if>
					</div>
				</div>
			</html:form>
