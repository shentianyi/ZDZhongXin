...
<script>
	...
	function doClear() {
		...
		$("#moveTime").datebox('clear');
		$("#moveEndTime").datebox('clear');
		$("[name='query.vin']").val("");
...
<body class="h-100 public" onLoad="doLoad('moveTime,moveEndTime')">
	<div class="public-main abs">
		<div class="ly-contai rel">
			<html:form action="/supervisorMoveStock.do" styleId="/supervisorMoveStockForm" method="post" onsubmit="return false">
				...
				<input type="hidden" id="onehiId" value="<c:out value="${supervisorMoveStockForm.query.oneBankId}"/>"/>
				<input type="hidden" id="twohiId" value="<c:out value="${supervisorMoveStockForm.query.twoBankId}"/>"/>
				<input type="hidden" id="threehiId" value="<c:out value="${supervisorMoveStockForm.query.threeBankId}"/>"/>
				...
				<div class="public-main-input ly-col-2 hidden abs" style="height: 165px;margin-top: -50px;">
					<div class="ly-input-w" style="height: 110px;">
						...
						<div class="ly-row clearfix">
						<div class="ly-col fl" style="width:33%;display:none">
	                    ...
	                    <div class="ly-col fl" style="width:66%;display:none" >
	                        	<div class="label block fl hidden" style="width:20%;margin-left: -3px;">移动时间：</div>
	                        	<div class="input block fl hidden" style="width:80%;padding-left:28px;">
		                        	<input style="width:24%;" class="ly-bor-none" id="moveTime" name="query.moveTime" type="text" value='<fmt:formatDate value="${supervisorMoveStockForm.query.moveTime }" pattern="yyyy-MM-dd"/>'/>
	                        		<span>至</span>
		                        	<input style="width:24%;" class="ly-bor-none" id="moveEndTime" name="query.moveEndTime" type="text" value='<fmt:formatDate value="${supervisorMoveStockForm.query.moveEndTime }" pattern="yyyy-MM-dd"/>'/>
	                        	</div>
	                    </div>
	                    </div>
					</div>
							...
							<tbody class="t-tbody">
								<logic:iterate name="list" id="row" indexId="index">
									<tr class="t-tr" <c:if test="${row.isread==1}">style="color:red;"</c:if>>
										...
										<td class="t-td"><fmt:formatDate value="${row.moveTime}" pattern="yyyy-MM-dd HH:mm:ss" /></td>
										<td class="t-td"><c:out value="${row.moveaddress}"/></td>
				...
				<div class="public-main-footer hidden abs">
					...
					<div class="public-main-footer-pagin fr">
						<thumbpage:tools className="<%=ThumbPageConstants.CLASSNAME_DEFAULT.getCode()%>" 
  							tableName="findList" action="/supervisorMoveStock.do?method=findList" />
					</div>
				</div>
			</html:form>
		</div>
