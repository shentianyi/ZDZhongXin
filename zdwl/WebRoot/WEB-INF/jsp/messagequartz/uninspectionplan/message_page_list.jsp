...
.oneBankId, .twoBankId, .threeBankId, .prodirectorceId, .cityId {
	margin: 3% 10% ;
	width: 50% ;
    height: 64%;
}
...
<body class="h-100 public" onLoad="doLoad('beginTime,endTime')">
	<div class="public-main abs">
		<div class="ly-contai rel">
			<html:form action="/unInspectionPlan.do" styleId="/unInspectionPlanForm" method="post" onsubmit="return false">
				...
				<input type="hidden" id="onehiId" value="<c:out value="${unInspectionPlanForm.query.oneBankId}"/>"/>
				<input type="hidden" id="twohiId" value="<c:out value="${unInspectionPlanForm.query.twoBankId}"/>"/>
				<input type="hidden" id="threehiId" value="<c:out value="${unInspectionPlanForm.query.threeBankId}"/>"/>
				...
				<div class="public-main-input ly-col-2 hidden abs" style="height: 165px;margin-top: -50px;">
					<div class="ly-input-w" style="height: 110px;">
						...
						<div class="ly-row clearfix">
							<div class="ly-col fl" style="width:33%;">
								<div class="label block fl hidden">应检查经销店名称：</div>
								<div class="input block fl hidden">
							...
							<div class="ly-col fl" style="width:33%;">
								<div class="label block fl hidden">风控专员：</div>
								<div class="input block fl hidden">
									<html:text styleClass="ly-bor-none" property="query.director" styleId="query.director"/>
								</div>
						   </div>
						</div>
						<div class="ly-row clearfix">
	                    <div class="ly-col fl" style="width:66%;" >
	                        	<div class="label block fl hidden" style="width:20%;margin-left: -3px;">未执行时间：</div>
	                        	<div class="input block fl hidden" style="width:80%;padding-left:28px;">
		                        	<input style="width:24%;" class="ly-bor-none" id="beginTime" name="query.beginTime" type="text" value='<fmt:formatDate value="${unInspectionPlanForm.query.beginTime }" pattern="yyyy-MM-dd"/>'/>
	                        		<span>至</span>
		                        	<input style="width:24%;" class="ly-bor-none" id="endTime" name="query.endTime" type="text" value='<fmt:formatDate value="${unInspectionPlanForm.query.endTime }" pattern="yyyy-MM-dd"/>'/>
	                        	</div>
	                    </div>
	                    </div>
					</div>
						...
						<table class="t-table" border="0" cellspacing="0" cellpadding="0">
							<thead class="t-thead">
								<tr class="t-tr">
									...
									<th class="t-th">风控专员</th>
									<th class="t-th">应检查经销店名称</th>
									<th class="t-th">金融机构</th>
									<th class="t-th">品牌</th>
									<th class="t-th">预计开始日期</th>
							...
							<tbody class="t-tbody">
								<logic:iterate name="list" id="row" indexId="index">
									<tr class="t-tr" <c:if test="${row.isread==1}">style="color:red;"</c:if>>
										...
										<td class="t-td"><c:out value="${row.director}" /></td>
										...
										<td class="t-td"><fmt:formatDate value="${row.plandate}" pattern="yyyy-MM-dd HH:mm:ss" /></td>
				...
				<div class="public-main-footer hidden abs">
					...
					<div class="public-main-footer-pagin fr">
						<thumbpage:tools className="<%=ThumbPageConstants.CLASSNAME_DEFAULT.getCode()%>" 
  							tableName="findList" action="/unInspectionPlan.do?method=findList" />
					</div>
				</div>
			</html:form>
		</div>
