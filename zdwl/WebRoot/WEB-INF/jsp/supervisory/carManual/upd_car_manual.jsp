...
//执行保存操作
function doSave(){
	var value = document.getElementById("actualAddress.distribid").value;
	if(value == "-1"){
		alert("请选择经销商");
		return false;
	}
	var value = document.getElementById("actualAddress.agreement_address").value;
	if(value == ""){
		alert("请填写协议地址");
		return false;
	}
	var value = document.getElementById("actualAddress.actual_address").value;
	if(value == ""){
		alert("请填写实际监管地址");
		return false;
	}
	var value = document.getElementById("actualAddress.relationship").value;
	if(value == ""){
		alert("请填写关系");
		return false;
	}
	var value = document.getElementById("actualAddress.isreport").value;
	if(value == "-1"){
		alert("请选择是否上报");
		return false;
	}
	var value = document.getElementById("actualAddress.distance").value;
	if(value == ""){
		alert("请填写距离");
		return false;
	}
...
//执行返回列表操作
function doReturn(){
	location = "<url:context/>/carManual.do?method=carManualList";
}
...
<html:hidden property="carmanual.id" styleId="carmanual.id"/>
<input type="hidden" name="method" id="method" value="updCarManual">
<table class="formTable">
	...
	<tr>
		...
		<td class="codeCol">
			<form:select property="actualAddress.isreport" styleId="actualAddressisreport">
				<html:option value="-1">请选择</html:option>
