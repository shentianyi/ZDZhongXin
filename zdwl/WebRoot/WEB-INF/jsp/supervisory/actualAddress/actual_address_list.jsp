...
//进入新增页面
function goAdd(){
	location="<url:context/>/actualAddress.do?method=addActualAddressEntry";
}

//进入修改页面
function goUpd(id){
	location = "<url:context/>/actualAddress.do?method=updActualAddressEntry&actualAddress.id="+id;
}

//执行删除操作
function doDel(id){
	if(confirm("删除后数据不可恢复\n是否继续？")){
		location="<url:context/>/actualAddress.do?method=delActualAddress&actualAddress.id="+id;
	}
}
...
<div class="public-bar hidden">
	<div class="ly-contai clearfix">
		<div class="public-bar-crumbs fl hidden">
            ...
            > 
            <a class="crumbs-link" href="#">实际监管地址</a>
        </div>
	</div>
</div>
<div class="public-main abs">
	<div class="ly-contai rel">
		...
		<input name="method" id="method" type="hidden" value="actualAddressList"/>
				...
				<div class="ly-row clearfix">
					<div class="ly-col fl">
                        ...
                        <div class="input block fl hidden">
							<form:select styleClass="ly-bor-none" property="actualAddressquery.distribid" styleId="actualAddressquery.distribid">
                    ...
                    <div class="ly-col fl">
                        <div class="label block fl hidden">关系：</div>
                        <div class="input block fl hidden">
                            <html:text styleClass="ly-bor-none" property="actualAddressquery.relationship" styleId="actualAddressquery.relationship"/>
                        </div>
                    </div>
                    <div class="ly-col fl">
                        <div class="label block fl hidden">是否上报：</div>
                        <div class="input block fl hidden">
							<form:select styleClass="ly-bor-none" property="actualAddressquery.isreport" styleId="actualAddressquery.isreport">
								<html:option value="-1">请选择</html:option>
				...
				<table class="t-table" border="0" cellspacing="0" cellpadding="0">
					<thead class="t-thead">
						<tr class="t-tr">
							...
							<th class="t-th">协议地址</th>
							<th class="t-th">实际监管地址</th>
							<th class="t-th">关系</th>
							<th class="t-th">是否上报</th>
					...
					<tbody class="t-tbody hidden">
						<logic:iterate name="list" id="row" indexId="index">
							<tr class="t-tr">
								...
								<td class="t-td"><c:out value="${row.agreement_address}" /></td>
								<td class="t-td"><c:out value="${row.actual_address}" /></td>
								<td class="t-td"><c:out value="${row.relationship}" /></td>
								<td class="t-td">
									<c:if test="${row.isreport == 1}">
										是
									</c:if>
									<c:if test="${row.isreport == 2}">
										否
									</c:if>
								</td>
		...
		<div class="public-main-footer hidden abs">
			...
			<div class="public-main-footer-pagin fr">
				<thumbpage:tools className="<%=ThumbPageConstants.CLASSNAME_DEFAULT.getCode()%>" tableName="actualAddressList" action="/actualAddress.do?method=actualAddressList"/>
			</div>
		</div>
		</html:form>
	</div>
