...
function doLoad(){
	...
	//进入新增页面
	function goAdd() {
		location = "<url:context/>/supervisor/checkStock.do?method=preAdd";
	}
	...
	function doDel(id) {
		if (confirm("删除后数据不可恢复\n是否继续？")) {
			location = "<url:context/>/supervisor/checkStock.do?method=delete&checkStock.id=" + id;
		}
	}
	...
	function goAlreadyApproval(){
		location.href="<url:context/>/supervisor/checkStock.do?method=findList&query.pageType=2";
	}
	
	function doSubmit(id){
		if(confirm("确定要提交吗？")){
			location.href="<url:context/>/supervisor/checkStock.do?method=submit&checkStock.id="+id;
		}
	}
	
	function approval(id){
		location.href="<url:context/>/supervisor/checkStock.do?method=preApproval&query.id="+id;
	}
	
	function detail(id){
		location.href="<url:context/>/supervisor/checkStock.do?method=detailPage&query.id="+id;
	}
	
	function updateDealer(id){
		location.href="<url:context/>/supervisor/checkStock.do?method=dealerList&query.id="+id;
	}
</script>
</head>
...
<div class="add_nav">
	<span class="add_nav_acolor"> 
            ...
            &gt;
            <a class="crumbs-link" style="color:#929291;" href="#">查库凭证申请</a>
         </span>
</div>
...
<div class="public-main abs">
	<div class="ly-contai rel">
		...
		<html:form action="/supervisor/checkStock.do" styleId="checkStockForm" method="post" onsubmit="return false">
					...
					<table class="t-table" border="0" cellspacing="0" cellpadding="0">
						<thead class="t-thead">
							<tr class="t-tr">
								...
								<th class="t-th">申请原因</th>
								<th class="t-th">查库批次</th>
						...
						<tbody class="t-tbody hidden">
							<logic:iterate name="list" id="row" indexId="index">
								<tr class="t-tr">
									...
									<td class="t-td"><c:out value="${row.reason }"></c:out></td>
									<td class="t-td"><c:out value="${row.count }"></c:out></td>
									...
									<td class="t-td">
										...
										<c:if test="${row.approvalState==0 }">
											<a href="javascript:updateDealer('<c:out value="${row.id }"/>')">修改经销商</a>
			...
			<div class="public-main-footer hidden abs">
				...
				<div class="public-main-footer-pagin fr">
					<thumbpage:tools className="<%=ThumbPageConstants.CLASSNAME_DEFAULT.getCode()%>" tableName="list" action="/supervisor/checkStock.do?method=findList" />
				</div>
			</div>
		</html:form>
	</div>
