...
<script>
	...
	function goAlreadyApproval(){
		location.href="<url:context/>/carOperation.do?method=movefindList&carOperationquery.pageType=2";
	}
	
	function approval(id){
		location.href="<url:context/>/carOperation.do?method=movePreApproval&carOperation.id="+id;
	}
	
	function detail(id){
		location.href="<url:context/>/carOperation.do?method=moveDetailPage&carOperation.id="+id;
	}
...
<body>
	...
	<div class="title">监管物移动申请</div>
	<div class="pagebodyOuter">
		<div class="pagebodyInner">
			<!--页签-->
			<div class="tagbarMenu">
				<label class="focusLable"> <a href="javascript:void(0);">待审批</a>
				</label> <label class="blurLable"> <a
					href="javascript:goAlreadyApproval();">已审批</a>
				</label>
			</div>

			<div class="tagbarBody">
				<html:form action="/carOperation" styleId="carOperationForm" method="post" onsubmit="return false">
					<input name="method" id="method" type="hidden" value="movefindList" />
					<div class="dvScroll">
								...
								<logic:iterate name="list" id="row" indexId="index">
									<tr class="listTr_a">
										...
										<td align="center" nowrap="true" class="right-border-none">
											...
											<c:if test="${carOperationForm.carOperationquery.currRole != sc }">
					...
					<thumbpage:tools className="<%=ThumbPageConstants.CLASSNAME_DEFAULT.getCode()%>" tableName="movefindList" action="/carOperation.do?method=movefindList" />
