...
// 执行保存操作
function doSave() {
	if (selectDealer.dealerIds.length < 1) {
		alert("请选择要拆绑的经销商");
		return false;
	}
	...
	if (cm) {
		for (var i = 0; i < cm.length; i++) {
			...
			cc.push(new changeMoney(
					dealerId,
					$("#changeMoney"+dealerId).numberbox('getValue'),
					$("#changeMode"+dealerId).val(),
					$("#changeMoneyDate"+dealerId).datebox('getValue'))
			);
...
function showDealer(ids) {
	...
	if (ids) {
		var tab = $(".jxsInfo");
		var html = "<tr class='newDealer'><td colspan='4' ><br/><h3>经销商信息(拆出)</h3></td></tr>";
		for (var i = 0; i < ids.length; i++) {
			var newDealer = getDealer(ids[i]);
			console.info(newDealer);
			if (newDealer != null) {
				html += "<tr class='newDealer'><td colspan='4'><hr/></td></tr>"
						...
						+ "<td class='codeCol2'><input dealerid='"+newDealer.id+"' type='text' id ='changeMoney"
						+ newDealer.id
						+ "' name='changeMoney'/></td>"
						+ "<td class='nameCol2'><font color='#FF0000'>*</font>变更付费方式：</td>"
						+ "<td class='codeCol2'>" 
						+ "<select id='changeMode"+(newDealer.id+"'")+" name='changeMode'>"
							+"<option "+(newDealer.payMode=='月'?"selected='selected'":"")+" value='1'>月</option>"
							+"<option "+(newDealer.payMode=='季度'?"selected='selected'":"")+" value='2'>季度</option>"
							+"<option "+(newDealer.payMode=='半年'?"selected='selected'":"")+" value='3'>半年</option>"
							+"<option "+(newDealer.payMode=='年'?"selected='selected'":"")+"  value='4'>年</option>"
						+"</select>"
						+"</td>"
						+ "</tr>"
						+"<tr class='newDealer'>"
							...
							+ "<td class='codeCol2'>"
							+ "<input id='changeMoneyDate"+(newDealer.id+"'")+" name='changeMoneyDate' type='text'> </input>"
							+ "</td>"
						+"</tr>";
				if (newDealer.bindId) {
					html += "<tr class='newDealer'><td colspan='4'><br/><h3>经销商信息（保留）：</h3></td></tr>";
					var bindIds = newDealer.bindId.split(",");
					for (var i = 0; i < bindIds.length; i++) {
						newDealer = getDealer(bindIds[i]);
						html += "<tr class='newDealer'><td colspan='4'><hr/></td></tr>"
								+ "<tr class='newDealer'>"
								+ "<td class='nameCol2'>经销商名称：</td>"
								+ "<td class='codeCol2'>"
