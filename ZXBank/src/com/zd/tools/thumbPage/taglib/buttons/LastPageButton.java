 ...
 * 最后页按钮
 */
public class LastPageButton extends PageButton {
	
	//构造函数
	public LastPageButton(PageContext context,String tableName){
		super(context,tableName);
		
		//子类设置的按钮属性
		this.disabled = false;	//按钮是否不可用
		this.type = TYPE_BUTTON;	//按钮类型－按钮或图片
		this.cName = "最末页";	//按钮中文名（页面显示名）
		this.clickEvent = "to_lastPage('"+tableName+"')";	//按钮触发时间（对应匹配的JS）
		...
		//图片类型按钮使用的相应属性
		this.normalImageUrl = webroot + "/images/table/lastPage.gif";	//正常图片路径
		this.disabledImageUrl = webroot + "/images/table/lastPageDisabled.gif";	//不可用时图片路径
	}
