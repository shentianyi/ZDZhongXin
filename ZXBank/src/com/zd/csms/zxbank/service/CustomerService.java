...
public class CustomerService extends ServiceSupport {
	...
	public List<Customer> findcustallList(Customer customer,
			IThumbPageTools tools) {
		return idao.findcustallList(customer, tools);
	}

	// 远程数据与本地数据比对
	public void autoUpdateCust(List<CustomerFar> bankList, Customer customer)
			throws Exception {
		...
		if (bankList != null && bankList.size() > 0)
			for (CustomerFar customerFar : bankList) {
				...
				for (int i = 0; i < list.size(); i++) {
					if (list.get(i).getCustNo().trim()
							.equals(customerFar.getEcifCode().trim())) {
...
//						update(customerFar,customer.getCustOrganizationcode());
	...
	public void add(CustomerFar customerFar, String orgCode) {
		...
		cus.setCustCreateDate(DateUtil.getStringFormatByDate(date,
				"yyyy-MM-dd HH:mm:ss"));
		cus.setCustOrganizationcode(orgCode);
		idao.add(cus);
	}
